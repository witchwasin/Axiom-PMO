...
import { getTableRowsAfterHeading, getIdsFromRows, type TableRow } from "../markdown/table-parser.js";
...
export function testUserSourcePath(relativePath: string): boolean {
  return /^(source|MOM|REQ|Transcript|Others)[\\/]/.test(relativePath);
}

export function testGovernedPlaceholders(
...
export function testProjectSourceSection(
  acc: ResultAccumulator,
  catalog: ValidationRules | undefined,
  ...
  if (reqRows.length === 0) {
    ...
    for (const id of projectReqIds) idCounts.set(id, (idCounts.get(id) ?? 0) + 1);
    ...
    if (isFullSpecDepth) {
      const validReqTypes = (policyEnums["requirement_types"] as string[]) ?? [];
  ...
  const checkpoints = (ctx.policy["approval_checkpoints"] as Record<string, unknown>) ?? {};
  ...
  for (const approvalGate of ["Scope Approved", "Design Ready", "Release Approved"]) {
    const spec = checkpoints[approvalGate] as { gates?: string[]; exempt_modes?: string[] } | undefined;
    if (!spec) continue;
    const gates = spec.gates ?? [];
    const exemptModes = spec.exempt_modes ?? [];
    if (!gates.includes(gate)) continue;
    if (exemptModes.includes(mode)) continue;
...
export function testSensitiveFilenames(
  acc: ResultAccumulator,
  catalog: ValidationRules | undefined,
  allProjectFiles: string[],
  project: string,
): void {
  const sensitiveHitsGoverned: string[] = [];
  const sensitiveHitsSource: string[] = [];
  for (const file of allProjectFiles) {
    const rel = relative(project, file);
    for (const pattern of SENSITIVE_PATTERNS) {
      if (pattern.test(rel)) {
        if (testUserSourcePath(rel)) sensitiveHitsSource.push(rel);
        else sensitiveHitsGoverned.push(rel);
        break;
      }
    }
  }
