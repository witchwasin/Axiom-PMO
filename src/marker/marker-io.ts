...
export function readTextFileState(path: string): TextFileState {
  if (!existsSync(path)) {
...
export function writeTextFileAtomic(path: string, text: string, hasBom = false): void {
  ...
  try {
    ...
    if (existsSync(temp)) {
      try { import("node:fs").then(({ unlinkSync }) => unlinkSync(temp)); } catch {}
    }
    throw e;
  }
}

export function newAxiomBackup(path: string): string {
  ...
  try {
    ...
    throw new Error(`could not write a backup next to ${basename(path)}: ${(e as Error).message}`);
  }
  return candidate;
}

function formatInvariantStamp(d: Date): string {
...
export function getAxiomBackups(path: string): string[] {
  const directory = dirname(path) || ".";
