...
export function getReleaseRegistry(releaseText: string | null): ReleaseRegistry {
  const result: ReleaseRegistry = { releaseId: null, testIds: [], testRows: [] };
  if (!releaseText) return result;
  const rid = /^\s*>?\s*Release ID:\s*(REL-\d{3})\s*$/m.exec(releaseText);
  if (rid) result.releaseId = rid[1]!;
...
export function testTestSummary(
...
export function testTestEvidenceGitGroundTruth(
  acc: ResultAccumulator,
  catalog: ValidationRules | undefined,
  registry: ReleaseRegistry,
  project: string,
  ...
  const candidates: TableRow[] = [];
  for (const row of rows) {
    const result = (row["Result"] ?? "").trim().toLowerCase();
    if (result !== "passed") continue;
    if (testPlaceholderValue(row["Evidence"] ?? "")) continue;
    const ref = resolveReference(row["Evidence"]!, referenceTypesConfig, project, null, null, null, registry.testIds);
    if (ref.type !== "file" || !ref.resolved || ref.pathEscaped) continue;
    candidates.push(row);
  }
  ...
  for (const row of candidates) {
    ...
    if (ls.status !== 0 || !gitRelPath) continue;
...
export function testRtmTraceability(
  acc: ResultAccumulator,
  catalog: ValidationRules | undefined,
  project: string,
  referenceTypesConfig: ReferenceTypesConfig,
  ...
  let rtmDoc: { schema_version?: string; traceability?: RtmRow[] } | null = null;
  try {
