...
function getSkillFileMap(root: string): Record<string, string> {
...
export function buildPluginPackage(repoRoot: string, check: boolean): PluginPackageResult {
  const repo = resolve(repoRoot);
  const sourceRoot = join(repo, ".claude/skills");
  const mirrorRoot = join(repo, "skills");

  if (!existsSync(sourceRoot)) return { output: "[FAIL] PLUGIN-PKG-001 Source skills directory not found: .claude/skills\n", exitCode: 1 };
  ...
  if (check) {
    ...
    for (const key of sourceKeys) {
      if (!(key in mirror)) problems.push(`missing from skills/: ${key}`);
      else if (mirror[key] !== source[key]) problems.push(`content differs: ${key}`);
    }
    for (const key of Object.keys(mirror)) {
      if (!(key in source)) problems.push(`present in skills/ but not in .claude/skills/: ${key}`);
    }
