...
function assess(fixture, mode = "Standard") {
...
test("build readiness and demo readiness are separate", () => {
    ...
    assert.ok(demo.score.awarded < demo.score.total, `score=${demo.score.awarded}/${demo.score.total}`);
    assert.ok(!demo.semantic_review["is_approval"], "the review is never reported as an approval");
    assert.equal(demo.semantic_review["open_blockers"].filter((b) => b["origin"] === "review_finding").length > 0, true, "open actions and review findings are distinguishable by origin");
    assert.equal(demo.semantic_review["open_blockers"].filter((b) => b["finding_id"] === "OA-002").length, 0, "OA-002 is already represented by finding HF-005");
...
test("readiness is unknown, not true, when nobody has reviewed", () => {
    ...
    assert.equal(noReviewStages.verdicts["Ready to Start Development"], null, `got '${noReviewStages.verdicts["Ready to Start Development"]}'`);
    assert.ok(noReviewStages.verdict_reasons["Ready to Start Development"]?.includes("unknown"), `got '${noReviewStages.verdict_reasons["Ready to Start Development"]}'`);
...
test("a reviewed artifact changing is staleness too, not just the sources", () => {
    ...
    assert.ok(staleInputs.semantic_review["stale"], "editing BUILD-SPEC after review marks the review stale");
    assert.ok(String(staleInputs.semantic_review["stale_reason"]).includes("artifact"), `got '${staleInputs.semantic_review["stale_reason"]}'`);
});
test("open actions in HANDOFF.md block stages, not only review findings", () => {
    ...
    assert.ok(openAction.semantic_review["open_blockers"].filter((b) => b["origin"] === "open_action").length > 0, "the blocking open action is named in the output");
    assert.ok(openAction.score.awarded < openAction.score.total, `score=${openAction.score.awarded}/${openAction.score.total} with Ready to Demo = ${openAction.verdicts["Ready to Demo"]}`);
    assert.ok(openAction.verdicts["Ready to Demo"] === false && openAction.score.awarded < 100, "a project that blocks its own demo never scores full marks");
    const dims = openAction.score["dimensions"];
...
test("score caps reflect review absence, staleness, and blocking severity", () => {
    ...
    assert.ok(noReview.score.awarded <= 70, `score=${noReview.score.awarded}`);
    assert.ok(noReview.score["caps_applied"].some((c) => c["id"] === "review_absent_or_stale"));
    ...
    assert.ok(staleReview.semantic_review["stale"]);
    assert.ok(staleReview.score.awarded <= 70, `score=${staleReview.score.awarded}`);
    const blockedBuild = assess("tests/fixtures/invalid-handoff-open-before-build-critical");
    assert.ok(blockedBuild.score.awarded <= 49, `score=${blockedBuild.score.awarded}`);
    assert.equal(blockedBuild.verdicts["Ready to Start Development"], false);
    assert.ok(blockedBuild.score["caps_applied"].some((c) => c["id"] === "open_before_build_critical"));
});
test("a deterministic FAIL is always BLOCKED, whatever the score says", () => {
    ...
    assert.equal(Object.values(deterministic.verdicts).filter((v) => v !== false).length, 0, "every stage verdict is an explicit false, never unknown");
    const ownerGap = assess("tests/fixtures/invalid-handoff-workitem-generic-owner");
    assert.ok(ownerGap.score["caps_applied"].some((c) => c["id"] === "missing_owner_or_sequence"));
});
test("a clean Lite handoff reports honestly rather than perfectly", () => {
    ...
    assert.ok(lite.score.awarded <= 70, `score=${lite.score.awarded}`);
});
