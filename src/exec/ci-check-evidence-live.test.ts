...
function newCiEntry(name: string, commitSha: string, conclusion?: string, checkRunId?: string): TestEvidenceEntry {
  const raw: Record<string, unknown> = { type: "ci-check", name, commit_sha: commitSha };
...
test("ci-check live evidence: positive path, case sensitivity, check_run_id binding, ambiguity", (t) => {
  ...
  let foundCheckName: string | null = null;
  let foundCheckRunId: string | null = null;
  let foundFailingName: string | null = null;

  for (let depth = 1; depth < 9; depth++) {
    const sha = git("rev-parse", `HEAD~${depth}`);
    if (!sha.ok || !sha.stdout) break;
  ...
  if (rawAll.ok) {
    try {
      const allData: { check_runs: CheckRun[] } = JSON.parse(rawAll.stdout);
      const byName = new Map<string, string[]>();
      const successIdByName = new Map<string, string>();
      for (const run of allData.check_runs ?? []) {
        if (run.status !== "completed") continue;
        const n = run.name;
        if (!byName.has(n)) byName.set(n, []);
        byName.get(n)!.push(run.conclusion ?? "");
        if (run.conclusion === "success" && !successIdByName.has(n)) successIdByName.set(n, String(run.id));
      }
      let ambiguousName: string | null = null;
      let ambiguousSuccessId: string | null = null;
      let allSuccessDuplicateName: string | null = null;
      for (const [k, vals] of byName) {
        if (vals.length < 2) continue;
        const distinct = [...new Set(vals)];
        if (distinct.length > 1 && !ambiguousName) {
          ambiguousName = k;
          if (successIdByName.has(k)) ambiguousSuccessId = successIdByName.get(k)!;
        }
        if (distinct.length === 1 && distinct[0] === "success" && !allSuccessDuplicateName) allSuccessDuplicateName = k;
      }
