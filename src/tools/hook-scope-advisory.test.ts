...
import {
  mkdtempSync, mkdirSync, rmSync, writeFileSync, readFileSync, readdirSync, existsSync, symlinkSync, statSync,
} from "node:fs";
...
function newHookProject(sandbox: string, name: string, optIn: boolean, scope: string | null = DEFAULT_SCOPE): string {
  const dir = join(sandbox, name);
  mkdirSync(dir, { recursive: true });
  if (scope) writeFileSync(join(dir, "SCOPE.json"), scope, "utf8");
  if (optIn) {
...
function newPayload(project: string, filePath: string, tool = "Edit"): string {
...
test("hook advisory: SCOPE-DIFF repo-wide exemption parity", () => {
  ...
  try {
    ...
    const p = newHookProject(sandbox, "optin", true);
    const cases: Array<{ name: string; path: string; expect: "silent" | "reported" }> = [
...
test("hook advisory: it decides nothing", () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      hookCode,
      /permissionDecision|permission_decision|hookSpecificOutput/i,
      "no executable line in the hook emits a permission-decision field (the point is not that no case produced one, but that no case could)",
    );
...
test("hook advisory: no or malformed SCOPE.json", () => {
  ...
  try {
    ...
    assert.ok(
      r3.exitCode === 0 && r3.output === "",
      `a malformed SCOPE.json degrades to silence, not to a broken edit (exit=${r3.exitCode} ${r3.output})`,
    );
  } finally {
    rmSync(sandbox, { recursive: true, force: true });
  }
...
test("hook advisory: the shell shim", () => {
  ...
  try {
    ...
    const noNodeEnv: Record<string, string> = {
      AXIOM_NODE: "/nonexistent/node",
      CLAUDE_PLUGIN_ROOT: REPO_ROOT,
    };
    if (process.platform !== "win32") noNodeEnv.PATH = fakeBin;
    ...
    assert.match(
      r.text, /scope advisory/,
      `a JSON-escaped cwd is un-escaped, and the advisory actually fires (silence here means the shim looked for the opt-in at a path that does not exist: ${r.text})`,
    );
...
test("hook advisory: the registration", () => {
  ...
  const hooksJson = JSON.parse(readFileSync(HOOKS_JSON_PATH, "utf8")) as Record<string, unknown>;
  const preToolUse = (hooksJson["hooks"] as Record<string, unknown>)["PreToolUse"] as Array<Record<string, unknown>>;
  ...
  const firstHook = (first["hooks"] as Array<Record<string, unknown>>)[0]!;
  ...
  assert.ok(
    /report-only/i.test(String(hooksJson["description"])) && /opt-in/i.test(String(hooksJson["description"])),
    "...and the registration describes itself as report-only and opt-in",
  );
});
...
test("hook advisory: cross-plugin coexistence, no ordering dependency", () => {
  ...
  const hooksJson = JSON.parse(hooksJsonText) as Record<string, unknown>;
  const preToolUse = (hooksJson["hooks"] as Record<string, unknown>)["PreToolUse"] as Array<Record<string, unknown>>;
  const first = preToolUse![0]!;
  assert.match(String(first["matcher"]), /\|/, "the matcher does not claim tools exclusively (a regex OR, not a lock)");
  assert.doesNotMatch(
    hooksJsonText,
    /"(priority|order|precedence|sequence|before|after)"\s*:/i,
    "the registration names no priority, order, or precedence field",
  );
  const hookCode = hookSourceWithoutComments();
  assert.doesNotMatch(
    hookCode,
    /HOOK_(RESULT|OUTPUT|STATE)|PREVIOUS_HOOK|OTHER_HOOK/i,
    "the hook script reads no environment variable suggesting another hook's output",
  );
});
