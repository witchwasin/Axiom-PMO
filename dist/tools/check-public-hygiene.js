...
export function checkPublicHygiene(repoPath) {
    ...
    function testAllowedMatch(relativePath, pattern) {
        for (const entry of allowlist["allowed_matches"] ?? []) {
            if (entry["pattern"] !== pattern)
                continue;
            for (const allowedPath of entry["paths"] ?? []) {
                if (toRepoPath(relativePath) === toRepoPath(allowedPath))
                    return true;
            }
        }
        return false;
    ...
    if (existsSync(privatePatternsPath)) {
        ...
        let i = 0;
        for (const pattern of privateConfig["private_patterns"] ?? []) {
            i++;
    ...
    const selfExcludedPaths = ["pmo-config/public-hygiene-allowlist.json", "scripts/check-public-hygiene.ps1", "src/tools/check-public-hygiene.ts", "dist/tools/check-public-hygiene.js"];
    const checks = [
    ...
    for (const file of uniqueTracked) {
        ...
        if (selfExcludedPaths.includes(relativePath))
            continue;
        ...
        for (const check of checks) {
            ...
            if (!matched)
                continue;
            if (testAllowedMatch(relativePath, check.pattern))
                continue;
            problems.push(`${check.id}: ${check.description} in ${relativePath} (pattern: ${check.pattern})`);
        }
    }
    if (problems.length === 0)
        return { output: `Axiom-PMO Public Hygiene Check: ${repo}\nSummary: PASS=1 FAIL=0\n`, exitCode: 0 };
