...
export function getCanonicalGoldenText(text: string | null | undefined): string {
  ...
  normalized = normalized.replace(/(?<!\\)\\u([0-9a-fA-F]{4})/g, (_m, hex: string) =>
...
export function testGoldenMatch(expected: string, actual: string): boolean {
  return getCanonicalGoldenText(expected) === getCanonicalGoldenText(actual);
}

export function getGoldenDiffReport(
  expected: string,
  actual: string,
  maxDifferences = 12,
): string[] {
  ...
  if (report.length === 0) {
    report.push(
      `      no differing line found; expected ${expectedLines.length} line(s), actual ${actualLines.length}`,
    );
  }
  return report;
}
