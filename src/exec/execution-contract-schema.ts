...
export function getExecutionFileDigest(path: string): string | null {
  if (!existsSync(path)) return null;
  return createHash("sha256").update(readFileSync(path)).digest("hex").toLowerCase();
}

export function readExecutionContractPolicy(frameworkRoot: string): Record<string, unknown> {
  const policyPath = join(frameworkRoot, "pmo-config/execution-contract-policy.json");
  if (!existsSync(policyPath)) throw new Error(`Missing runtime execution-contract policy config: ${policyPath}`);
...
export function readExecutionContract(path: string): ContractRead {
  const result: ContractRead = { present: false, valid: false, error: null, document: null, digest: null, path };
  ...
  try {
    ...
    result.error = `EXECUTION-CONTRACT.json is not valid JSON: ${(e as Error).message}`;
    return result;
  }
  ...
  const allowed = (doc["allowed_paths"] as unknown[]) ?? [];
  if (allowed.length === 0) {
...
export function readExecutionResult(path: string): ResultRead {
  const result: ResultRead = { present: false, valid: false, error: null, document: null, path };
  if (!existsSync(path)) return result;
  result.present = true;
  ...
  try {
    ...
    result.error = `EXECUTION-RESULT.json is not valid JSON: ${(e as Error).message}`;
    return result;
  }
...
export function resolveTestEvidenceEntries(result: Record<string, unknown>, policy: Record<string, unknown>): TestEvidenceEntry[] {
  const entries: TestEvidenceEntry[] = [];
  const rawEntries = (result["test_evidence"] as Array<Record<string, unknown>>) ?? [];
  const adapters = (policy["test_evidence_adapters"] as Array<Record<string, unknown>>) ?? [];
  ...
  for (const item of rawEntries) {
    ...
    if (adapter) {
      for (const required of (adapter["requires"] as string[]) ?? []) {
        if (item[required] === undefined || String(item[required] ?? "").trim() === "") missingFields.push(required);
      }
    }

    let provenance = "agent-claimed";
    if (adapter && String(adapter["provenance"] ?? "").trim() !== "") provenance = String(adapter["provenance"]);
...
export function readDecisionLog(projectPath: string): DecisionLog {
  const path = join(projectPath, "decision-log.md");
  if (!existsSync(path)) return { present: false, rows: [], path };
...
export function resolveDecisionRecord(projectPath: string, decisionRef: string | null | undefined): DecisionRecord {
  const result: DecisionRecord = { found: false, row: null, reason: null, logPath: null };
...
export function readDecisionAuthorityBindings(row: TableRow | null): Array<Record<string, string>> {
  const bindings: Array<Record<string, string>> = [];
  if (!row) return bindings;
  ...
  for (const payload of payloads) {
    ...
    if (Object.keys(fields).length > 0) bindings.push(fields);
  }
  return bindings;
}

export function testDecisionAuthorityBinding(
  row: TableRow | null,
  claimType: string,
  workItemId: string,
  ...
  for (const b of bindings) {
    if (!("type" in b)) { lastReason = "a binding on the decision record names no claim type"; continue; }
    if (b["type"] !== claimType) { lastReason = `the decision record authorizes '${b["type"]}', not '${claimType}'`; continue; }
    if (!("work_item" in b)) { lastReason = "the decision record's binding names no work_item"; continue; }
    if (b["work_item"] !== workItemId) { lastReason = `the decision record authorizes work item '${b["work_item"]}', not '${workItemId}'`; continue; }
    if (!("contract" in b)) { lastReason = "the decision record's binding names no contract digest"; continue; }
    if (b["contract"]!.toLowerCase() !== contractSha256.toLowerCase()) { lastReason = "the decision record authorizes a different contract digest than the one being verified"; continue; }
    if (testName) {
      if (!("test" in b)) { lastReason = "the decision record's binding names no test"; continue; }
      if (b["test"] !== testName) { lastReason = `the decision record approves evidence for test '${b["test"]}', not '${testName}'`; continue; }
    }
    if (evidenceSha256) {
      if (!("evidence" in b)) { lastReason = "the decision record's binding names no evidence digest"; continue; }
      if (b["evidence"]!.toLowerCase() !== evidenceSha256.toLowerCase()) { lastReason = "the decision record approves a different artifact digest than the evidence presented"; continue; }
    }
    return null;
  }
  return lastReason;
