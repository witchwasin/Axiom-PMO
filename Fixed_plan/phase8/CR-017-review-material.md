...
| **Windows NTFS junctions specifically** (a different reparse-point type than a symlink — `lstatSync(...).isSymbolicLink()` is not guaranteed to catch every junction the same way it catches a symlink) | — | ⚠️ **Not verified.** No dedicated test; nothing in this review confirms `SETUP-003`'s symlink check actually catches a real NTFS junction on a real Windows host. Flagging rather than asserting either way — this needs an actual Windows host to check, which nothing in local verification has covered. |
