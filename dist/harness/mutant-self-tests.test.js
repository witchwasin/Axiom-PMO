...
function run(actual) {
    return compareOutputs({ stdout: BASE_JSON, stderr: "", exitCode: 0 }, { stdout: actual, stderr: "", exitCode: 0 });
}
test("harness fails on changed exit code", () => {
    const r = compareOutputs({ stdout: BASE_JSON, stderr: "", exitCode: 0 }, { stdout: BASE_JSON, stderr: "", exitCode: 1 });
...
test("harness fails on reordered result", () => {
    const reordered = BASE_JSON.replace(`"rule_id": "STRUCT-001", "message": "ok"`, `"message": "ok", "rule_id": "STRUCT-001"`);
