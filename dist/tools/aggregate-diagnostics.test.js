...
test("no artifact content or free text reaches an event", () => {
    ...
    try {
        ...
        assert.ok(haveEvents && (!/\.md"/.test(allText) || /"artifact":"(PROJECT|DELIVERY|HANDOFF|RELEASE|RAID-log|decision-log)\.md"/.test(allText)), "no event carries a raw markdown/source path outside the governed allowlist");
    }
    finally {
...
test("rebuild is byte-for-byte identical (generated_at excepted)", () => {
    ...
    try {
        ...
        const strip = (r) => {
            const clone = JSON.parse(JSON.stringify(r.registry));
...
test("twenty reruns of one unfixed defect do not cross the candidate threshold", () => {
    ...
    try {
        ...
        const registry = JSON.parse(readFileSync(join(isolated, ".axiom/learning/FAILURE-PATTERNS.json"), "utf8"));
...
test("an alphabetic, potentially sensitive item_id is not retained", () => {
    const policy = JSON.parse(readFileSync(join(REPO_ROOT, "pmo-config/learning-policy.json"), "utf8"));
    assert.equal(normalizeItemId("D-123", policy.item_id_allowed_patterns), "D-#", "'D-123' (allowlisted shape) normalizes to 'D-#'");
    assert.equal(normalizeItemId("ACME-fraud-case", policy.item_id_allowed_patterns), "other", "'ACME-fraud-case' (no digits, not an allowlisted shape) is bucketed to 'other', not retained");
    ...
    assert.equal(normalizeItemId("D-123-extra", policy.item_id_allowed_patterns), "other", "a governed shape with extra trailing text ('D-123-extra') is bucketed to 'other', not partially matched");
    assert.equal(normalizeExecutionPath("development_handoff", policy.execution_path_allowed_values), "development_handoff", "'development_handoff' (valid enum) is retained");
    assert.equal(normalizeExecutionPath("customer-acme-secret-migration", policy.execution_path_allowed_values), "unknown", "a malformed execution_path value is normalized to 'unknown', not retained raw");
});
