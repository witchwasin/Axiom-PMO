...
function setChange(project, change) {
...
function setProjectDeclarations(project, ui = "dev_guided") {
...
test("M2: change control -- proposed harmless patch is auditable but does not block Handoff", () => {
    ...
    try {
        ...
        assert.ok(!result.some((r) => r.message.includes("sk-THIS-MUST-NOT-LEAK")), "CHANGE diagnostics leaked sensitive candidate text");
...
`;
        ...
        const mutatedStrategy = readFileSync(buildSpecPath, "utf8").replace(/\| Attach photo \| REQ-004 \|/, "| Attach photo | R-UNKNOWN |");
        ...
        const strictMutated = readFileSync(strictBuildSpec, "utf8").replace(/\| Scan lookup \| REQ-001 \| system \|/, "| Scan lookup | REQ-001 | <level> |");
