...
export function runValidationFixtures(repoRoot, verifyGolden, goldenMasterDir) {
    ...
    for (const c of CASES) {
        ...
        let results = [];
        try {
            results = JSON.parse(r.output).results ?? [];
        }
        catch {
            results = [];
        }
        ...
        if (!c.ShouldPass && c.Rule) {
            ...
            if (c.AllowedSecondaryRules !== undefined || c.ForbiddenRules !== undefined) {
                ...
                const observedBlockingRules = [...new Set(results.filter((x) => x.level === "FAIL" || (c.FailOnWarning && x.level === "WARN")).map((x) => x.rule_id))];
    ...
    if (verifyGolden) {
        if (goldenMismatches.length > 0) {
            ...
            for (const m of goldenMismatches)
                out.push(`  - ${m}`);
