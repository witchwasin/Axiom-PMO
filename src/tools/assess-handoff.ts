...
export function runAssessHandoff(repoRoot: string, projectPath: string, mode: string, format: "Text" | "Json"): { output: string; exitCode: number } {
  const project = resolve(projectPath);
  const cfg = importPmoConfig(repoRoot);
  const handoffPolicy = cfg.handoffPolicy;
  const scorePolicy = (handoffPolicy["score"] as Record<string, unknown>) ?? {};

  const gateResult = runPortedChain(repoRoot, project, mode as "Lite" | "Standard" | "Strict", "Handoff");
  ...
  if (reviewPresent) {
    try { review = JSON.parse(readFileSync(reviewPath, "utf8")); } catch { review = null; }
  }
  ...
  const recordedSourceDigest = review ? String((review["source_snapshot"] as Record<string, unknown> ?? {})["digest"] ?? "").trim().toLowerCase() : "";
  const recordedInputDigest = review ? String((review["review_inputs"] as Record<string, unknown> ?? {})["digest"] ?? "").trim().toLowerCase() : "";
  ...
  const openStatuses = ((reviewPolicy["closure_policy"] as Record<string, unknown>)?.["open_statuses"] as string[]) ?? [];
  const openFindings: Array<Record<string, unknown>> = review ? ((review["findings"] as Array<Record<string, unknown>>) ?? []).filter((f) => openStatuses.includes(String(f["status"]))) : [];

  const openActions: Blocker[] = [];
  ...
  const stageBlockingMap = (handoffPolicy["stage_blocking_map"] as Record<string, unknown>) ?? {};
  const stageBlockers: Record<string, Blocker[]> = {};
  for (const [stage, points] of Object.entries(stageBlockingMap)) {
    stageBlockers[stage] = allBlockers.filter((b) => (points as string[]).includes(b.blocking_point));
  }

  const contractValid = !deterministicFail;

  function stageVerdict(stage: string): boolean | null {
    if (!contractValid) return false;
    if ((stageBlockers[stage] ?? []).length > 0) return false;
    if (!reviewUsable) return null;
    return true;
  }
  function stageReason(stage: string): string {
    if (!contractValid) return "blocked: the contract has deterministic failures";
    const blockers = stageBlockers[stage] ?? [];
    if (blockers.length > 0) return `blocked by ${blockers.map((b) => b.finding_id).join(", ")}`;
    if (!reviewUsable) {
      if (review === null) return "unknown: no semantic review has been recorded";
      if (reviewStale) return "unknown: the semantic review is stale";
  ...
  const verdicts: Record<string, boolean | null> = { "Contract Valid": contractValid };
  const verdictReasons: Record<string, string> = { "Contract Valid": contractValid ? "no deterministic failures" : `${failures.length} deterministic FAIL diagnostic(s)` };
  for (const stage of stageNames) {
    verdicts[stage] = stageVerdict(stage);
    verdictReasons[stage] = stageReason(stage);
  }

  function dimensionPoints(id: string): number {
    const dim = ((scorePolicy["dimensions"] as Array<Record<string, unknown>>) ?? []).find((d) => String(d["id"]) === id);
    return dim ? Number(dim["points"]) : 0;
  }

  function dimensionScore(id: string, ruleIds: string[]): number {
    ...
    if (distinctFailingRules === 0) return points;
  ...
  const lensMap = (scorePolicy["lens_dimension_map"] as Record<string, unknown>) ?? {};
  const penaltyBySeverity = (scorePolicy["open_finding_penalty"] as Record<string, unknown>) ?? {};
  for (const finding of openFindings) {
    const lens = String(finding["lens"] ?? "");
    const dimensionId = lensMap[lens];
    if (typeof dimensionId !== "string" || !(dimensionId in dimensionScores)) continue;
  ...
  const blockingDimensionMap = (scorePolicy["blocking_point_dimension_map"] as Record<string, unknown>) ?? {};
  const actionPenalty = Number((scorePolicy["open_action_penalty"] as Record<string, unknown>)?.["points"] ?? 0);
  ...
  let score = rawScore;
  const appliedCaps: Array<Record<string, unknown>> = [];
  ...
  const dimensionsOut = ((scorePolicy["dimensions"] as Array<Record<string, unknown>>) ?? []).map((d) => ({
    id: String(d["id"]),
  ...
  if (format === "Json") return { output: JSON.stringify(assessment, null, 2) + "\n", exitCode: 0 };
  ...
  if (allBlockers.length > 0) {
    ...
    for (const point of (handoffPolicy["blocking_points"] as string[]) ?? []) {
      const atPoint = allBlockers.filter((b) => b.blocking_point === point);
      if (atPoint.length === 0) continue;
      lines.push(`  ${point}`);
  ...
  if (!reviewUsable) lines.push("The semantic review is not usable, so nothing here reflects a reader's judgement of whether the plan makes sense.");
