...
export function getTableRowsAfterHeading(text, headingPattern) {
...
export function getTableLinesAfterHeading(text, headingPattern) {
    const rx = toRegExp(headingPattern);
    const lines = text.split(/\r?\n/);
    let start = -1;
    for (let i = 0; i < lines.length; i++) {
...
export function getIdsFromRows(rows) {
    return rows.filter((r) => r["ID"]).map((r) => r["ID"].trim());
}
export function splitReferenceValues(value) {
    if (!value)
        return [];
    return value
        .split(",")
