...
function gitBlobBytes(repoRoot: string, revision: string): Buffer | null {
...
export function testExternallyObservedReviewBinding(
  review: Record<string, unknown>,
  reviewPath: string,
  gitRepoRoot: string,
  reviewArtifactPolicy: Record<string, unknown>,
): { verified: boolean; reason: string | null } {
  const binding = (reviewArtifactPolicy["externally_observed_binding"] as Record<string, unknown>) ?? {};
  ...
  const ghSpawn = { encoding: "utf8" as const, shell: process.platform === "win32" };
  ...
  const checkSuite = (run["check_suite"] as Record<string, unknown>) ?? {};
  ...
  let runsResponse: { workflow_runs?: Array<Record<string, unknown>> };
  try {
  ...
  const matchingWorkflowRuns = (runsResponse.workflow_runs ?? []).filter(
    (r) => String(r["path"] ?? "").replace(/@.*$/, "") === workflowRelPath,
  );
