...
test("canonical text hash matches PowerShell on a no-BOM markdown file", () => {
  assert.equal(
    getArtifactSha256("examples/LITE-BUGFIX/PROJECT.md"),
    "7de3ccdb1176753c6abafd9196af5c2c32fd891ef1fd4a291e00789ffd9adedd",
  );
});

test("canonical text hash matches PowerShell on a BOM JSON file", () => {
  // Expected digest re-verified after adding enums.requirement_types in Phase 2
  assert.equal(
    getArtifactSha256("pmo-config/policy.json"),
    "7c3730701e6c5a29020b94b0b41fa536c712fca6ae8a229bb93d5a1e0b47785b",
  );
});

test("binary hash matches PowerShell on an svg (unknown extension -> byte hash)", () => {
  assert.equal(
    getArtifactSha256("examples/DESIGN-SYSTEM-DEMO/DESIGN/BRAND/app-icon.svg"),
    "6156b4f04821de1c0f4b43ebf593db3bf1149b97265fb215d66bb6b41857b477",
  );
});
