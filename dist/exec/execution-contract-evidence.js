...
function isContained(projectPath, relPath) {
...
function xmlIntAttribute(node, name) {
...
export function testJUnitEvidence(entry, projectPath) {
    const result = { verified: false, reason: null, evidenceDigest: null };
    ...
    if (!containment.ok) {
        result.reason = containment.reason;
        return result;
    }
...
export function getGitHubOwnerRepo(remoteUrl) {
    if (!remoteUrl?.trim())
        return null;
...
function resolveGhOwnerRepo(repoRoot) {
    const r = spawnSync("gh", ["--version"], { encoding: "utf8" });
    if (r.status !== 0)
        return { ok: false, reason: "no GitHub API context available (gh CLI not found on PATH) -- cannot independently verify, so this is unverified rather than a pass" };
    const remote = gitRemote(repoRoot);
    const remoteUrl = remote.output;
    if (!remote.ok || !remoteUrl.trim())
        return { ok: false, reason: "could not resolve a git remote to query -- cannot independently verify" };
    const ownerRepo = getGitHubOwnerRepo(remoteUrl);
    if (!ownerRepo)
        return { ok: false, reason: "the git remote is not a recognizable GitHub URL -- cannot independently verify" };
    return { ok: true, ownerRepo };
}
export function testCiCheckEvidence(entry, gitRepoRoot) {
    const result = { verified: false, reason: null, evidenceDigest: null };
...
export function testRunnerExitEvidence(entry, projectPath, contractSha256, workItemId) {
    const result = { verified: false, reason: null, evidenceDigest: null };
    ...
    const containment = isContained(projectPath, relPath);
    if (!containment.ok) {
        result.reason = containment.reason.replace("path", "run_record_path");
        return result;
    }
    ...
    let record;
    try {
        record = JSON.parse(readFileSync(recordFull, "utf8"));
    }
    catch {
...
export function testEvidenceEntryVerified(entry, projectPath, gitRepoRoot, contractSha256, workItemId) {
    if (!entry.known)
        return { verified: false, reason: `unrecognized evidence type '${entry.type}'`, evidenceDigest: null };
    if (!entry.fieldsPresent)
        return { verified: false, reason: `missing required field(s): ${entry.missingFields.join(", ")}`, evidenceDigest: null };
    switch (entry.type) {
