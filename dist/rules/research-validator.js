...
function getResearchSnapshotIds(project) {
    const ids = [];
        ...
        const walk = (dir) => {
            for (const entry of readdirSync(dir)) {
                ...
                if (st.isDirectory())
                    ...
                    for (const m of entry.matchAll(/([A-Z]{2,6}-\d{6,8})/g))
                        ids.push(m[1]);
                }
            }
        };
...
function getProjectTextCache(project) {
    const parts = [];
...
function testResearchSourceResolvable(reference, project, snapshotIds, decisionIds, projectText) {
...
export function testResearchWorkflow(acc, catalog, project, gate, orchestrationPolicy, policyEnums, decisionIds, handoffPolicy) {
    const policy = orchestrationPolicy["research"] ?? {};
    ...
    if (!existsSync(reportPath)) {
        ...
        for (const section of requiredSections) {
            ...
            if (!pattern.test(reportText))
                structureProblems.push(section);
        }
    }
    let provenance = null;
    let claims = [];
    ...
    if (!existsSync(provenancePath)) {
        ...
        if (prov["schema_version"] !== "1.0")
            structureProblems.push("schema_version");
        claims = prov["claims"] ?? [];
        if (claims.length === 0)
            structureProblems.push("claims");
        for (const claim of claims) {
            if (/^RC-\d{3,}$/.test(String(claim["id"] ?? "")))
                claimIds.push(String(claim["id"]));
        }
        const rs = prov["research_status"];
        if (rs && String(rs).trim() !== "")
            researchStatus = String(rs).trim();
    }
    const provenanceProblems = [];
    let evidenceStatuses = policyEnums["evidence_statuses"] ?? [];
    if (evidenceStatuses.length === 0)
        evidenceStatuses = ["verified", "supported", "inferred", "missing", "conflict"];
    let sourceVerifications = policy["source_verifications"] ?? [];
    if (sourceVerifications.length === 0)
        sourceVerifications = ["verified", "unverified", "pending"];
    let freshnessModels = policy["freshness_models"] ?? [];
    if (freshnessModels.length === 0)
        freshnessModels = ["cutoff", "none"];
    let freshnessCutoff = null;
    if (provenance) {
        ...
        if (!freshness || String(freshness["model"] ?? "").trim() === "") {
            ...
            if (!/^\d{4}-\d{2}-\d{2}$/.test(cutoffRaw))
                provenanceProblems.push("freshness cutoff");
            else
    ...
    for (const claim of claims) {
        ...
        if (!evidenceStatuses.includes(String(claim["evidence_status"] ?? "")))
            provenanceProblems.push(`${id} evidence_status`);
        if (!reportHeadings.includes(String(claim["report_section"] ?? "").trim()))
            provenanceProblems.push(`${id} report_section`);
        ...
        for (const source of sources) {
            ...
            if (testResearchSourceResolvable(reference, project, snapshotIds, decisionIds, projectText))
                sourceOk = true;
            const title = String(source["title"] ?? "");
            const issuer = String(source["issuer"] ?? "");
            if (title.trim() === "" || testPlaceholderValue(title) || issuer.trim() === "" || testPlaceholderValue(issuer))
                provenanceProblems.push(`${id} source metadata`);
            if (typeof source["primary"] !== "boolean")
                provenanceProblems.push(`${id} primary`);
            if (!sourceVerifications.includes(String(source["verification"] ?? "")))
                provenanceProblems.push(`${id} verification`);
            ...
            if (freshnessCutoff !== null && (dateVal === undefined || String(dateVal ?? "").trim() === "")) {
                ...
                if (dateRaw !== "" && !/^\d{4}-\d{2}-\d{2}$/.test(dateRaw))
                    provenanceProblems.push(`${id} date`);
                else if (dateRaw !== "" && freshnessCutoff !== null && dateRaw < freshnessCutoff)
                    provenanceProblems.push(`${id} stale source`);
            }
        }
        if (!sourceOk)
            provenanceProblems.push(`${id} unresolvable source`);
    }
    const proposalProblems = [];
    const scopeProblems = [];
    const impactProblems = [];
    const proposalStatuses = policy["proposal_statuses"] ?? [];
    const proposalImpacts = policy["proposal_impacts"] ?? [];
    const proposalIds = [];
    ...
    if (existsSync(reportPath)) {
        ...
        for (const row of getTableRowsAfterHeading(reportText, "(?i)^\\s*#{1,4}\\s*Change Proposals\\s*$")) {
            ...
            if (!proposalStatuses.includes(status))
                proposalProblems.push(`${proposalId} status`);
            if (!proposalImpacts.includes(impact))
                proposalProblems.push(`${proposalId} impact`);
            ...
            if (blocksScope && status === "proposed" && ["Scope", "Design", "Handoff", "Release"].includes(gate))
                scopeProblems.push(proposalId);
        }
        if (researchStatus === "stopped" && ["Scope", "Design", "Handoff", "Release"].includes(gate))
            scopeProblems.push("stopped research");
        for (const row of getTableRowsAfterHeading(reportText, "(?i)^\\s*#{1,4}\\s*Impact Assessment\\s*$")) {
            ...
            if (!claimIds.includes(findingRef))
                impactProblems.push(`${findingRef} unknown claim`);
            if (mapsTo.trim() === "" || testPlaceholderValue(mapsTo))
                impactProblems.push(`${findingRef} maps_to`);
            if (proposedImpact.trim() === "" || testPlaceholderValue(proposedImpact))
                impactProblems.push(`${findingRef} impact`);
            if (!proposalStatuses.includes(status))
                impactProblems.push(`${findingRef} status`);
            if (status === "accepted") {
                if (!/^CP-\d{3,}$/.test(changeProposal))
                    impactProblems.push(`${findingRef} no proposal`);
                else if (!proposalIds.includes(changeProposal) || !["accepted", "rejected"].includes(proposalByStatus[changeProposal] ?? ""))
                    impactProblems.push(`${findingRef} undecided proposal`);
            }
        }
    }
    ...
    if (provenance) {
        ...
        if (providerUsed.trim() === "" || testPlaceholderValue(providerUsed))
            providerProblems.push("provider_used");
        const resolvedProviders = (policy["providers"] ?? []).filter((p) => p !== "none" && p !== "auto");
        if (declaredProvider === "auto") {
            if (!resolvedProviders.includes(providerUsed))
                providerProblems.push("provider agreement");
        }
        else if (declaredProvider && declaredProvider !== "none" && providerUsed !== declaredProvider) {
            providerProblems.push("provider agreement");
        }
        if (typeof provenance["provider_available"] !== "boolean")
            providerProblems.push("provider_available");
        if (typeof provenance["fallback_used"] !== "boolean")
            providerProblems.push("fallback_used");
        ...
        if (String(researchStatus ?? "").trim() === "")
            providerProblems.push("research_status");
        else if (!(policy["research_statuses"] ?? []).includes(researchStatus))
            providerProblems.push("research_status");
        if (researchStatus === "stopped") {
            const stopReason = String(provenance["stop_reason"] ?? "");
            const nextAction = String(provenance["next_action"] ?? "");
            if (stopReason.trim() === "" || testPlaceholderValue(stopReason))
                providerProblems.push("stop_reason");
            if (nextAction.trim() === "" || testPlaceholderValue(nextAction))
                providerProblems.push("next_action");
        }
        ...
        if (fallbackUsed === true && providerAvailable !== false)
            providerProblems.push("fallback without unavailable provider");
        ...
        if (!retrievedAtOk)
            providerProblems.push("retrieved_at");
    }
    const externalizationProblems = [];
    const externalProviders = policy["external_providers"] ?? [];
    const declaredProvider = getProjectOrchestrationDeclarations(project).researchProvider;
    if (externalProviders.includes(declaredProvider ?? "")) {
        let extRef = "";
        if (provenance)
            extRef = String(provenance["externalization"] ?? "");
        let approvedExt = null;
        const registryPath = join(project, String(orchestrationPolicy["externalization"]?.["registry"] ?? "EXTERNALIZATION.json"));
        if (existsSync(registryPath)) {
            try {
                const extDoc = JSON.parse(readFileSync(registryPath, "utf8"));
                for (const entry of extDoc.entries ?? []) {
                    if (entry.status === "approved" && entry.id === extRef)
                        approvedExt = entry;
                }
            }
            catch { }
        ...
        if (approvedExt === null) {
            ...
            if (!providerOk)
                externalizationProblems.push("provider mismatch");
        }
    }
    if (structureProblems.length)
        addResult(acc, catalog, "FAIL", `Guided research artifacts are missing or incomplete: ${[...new Set(structureProblems)].join(", ")}`, { ruleId: "RESEARCH-002", artifact: "RESEARCH/RESEARCH.md" });
    else
        addResult(acc, catalog, "PASS", "Guided research artifacts declare the required contract", { ruleId: "RESEARCH-002" });
    if (provenanceProblems.length)
        addResult(acc, catalog, "FAIL", `Research provenance is missing or unresolvable: ${[...new Set(provenanceProblems)].join(", ")}`, { ruleId: "RESEARCH-003", artifact: "RESEARCH/PROVENANCE.json" });
    else
        addResult(acc, catalog, "PASS", "Every material research claim maps to a resolvable source", { ruleId: "RESEARCH-003" });
    if (proposalProblems.length || impactProblems.length)
        addResult(acc, catalog, "FAIL", `Research change proposals or impact assessments lack valid Human decisions: ${[...new Set([...proposalProblems, ...impactProblems])].join(", ")}`, { ruleId: "RESEARCH-004", artifact: "RESEARCH/RESEARCH.md", field: "Change Proposals" });
    else
        addResult(acc, catalog, "PASS", "Research change proposals carry valid Human decision evidence", { ruleId: "RESEARCH-004" });
    if (scopeProblems.length)
        addResult(acc, catalog, "FAIL", `Scope cannot be approved with unresolved accepted-impact research proposals: ${[...new Set(scopeProblems)].join(", ")}`, { ruleId: "RESEARCH-005", artifact: "RESEARCH/RESEARCH.md", field: "Change Proposals" });
    else if (claims.length > 0)
        addResult(acc, catalog, "PASS", "No unresolved accepted-impact research proposal blocks this gate", { ruleId: "RESEARCH-005" });
    if (providerProblems.length)
        addResult(acc, catalog, "FAIL", `Research provider availability is not recorded truthfully: ${[...new Set(providerProblems)].join(", ")}`, { ruleId: "RESEARCH-006", artifact: "RESEARCH/PROVENANCE.json" });
    else
        addResult(acc, catalog, "PASS", "Research provider availability and fallback are recorded truthfully", { ruleId: "RESEARCH-006" });
    if (externalizationProblems.length)
        addResult(acc, catalog, "FAIL", `External research providers must cite a binding approved externalization entry: ${[...new Set(externalizationProblems)].join(", ")}`, { ruleId: "RESEARCH-007", artifact: "RESEARCH/PROVENANCE.json", field: "externalization" });
    else
        addResult(acc, catalog, "PASS", "Research provider usage cites approved externalization evidence", { ruleId: "RESEARCH-007" });
}
