...
export function preparePublicRelease(repoRoot: string, runSuite: boolean): ReleaseResult {
  ...
  if (runSuite) {
    ...
    if (doctorCode !== 0) problems.push(`Check 'doctor' failed (exit ${doctorCode})`);
    out.push(`doctor: exit ${doctorCode}`);

    const fixturesResult = runValidationFixtures(repo, true);
    if (fixturesResult.exitCode !== 0) problems.push(`Check 'fixtures' failed (exit ${fixturesResult.exitCode})`);
