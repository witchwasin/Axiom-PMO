...
import {
  mkdtempSync, mkdirSync, rmSync, writeFileSync, readFileSync, readdirSync, cpSync, existsSync,
} from "node:fs";
...
function newReviewFixture(mode: "Lite" | "Standard" | "Strict"): string {
...
function exportContract(dir: string): void {
...
interface ExecEnvelope {
  ...
  results: ExecRow[];
  execution_verification: { verdict: string };
}

function verifyResult(dir: string, preflight = false): ExecEnvelope {
  ...
  return r.envelope as unknown as ExecEnvelope;
}
...
function rows(env: ExecEnvelope, ruleId: string, level = "FAIL"): ExecRow[] {
...
function baseResult(dir: string, overrides: Record<string, unknown> = {}): Identity {
...
test("artifact-observed promoted: AREV-003 not raised, verdict pass", () => {
  ...
  try {
    ...
    newReview(dir, id);
    const resultPath = join(dir, ".execution/D-001/EXECUTION-RESULT.json");
    const resultDoc = JSON.parse(readFileSync(resultPath, "utf8")) as { authority_claims: unknown[] };
...
test("decision added within verified range: AREV-006 even though DEC-200 resolves", () => {
  ...
  try {
    ...
    writeFileSync(join(dir, ".execution/D-001/EXECUTION-RESULT.json"), JSON.stringify(doc));
    const identity: Identity = { digest, head, baseSha: String(contract["base_sha"]) };
...
test("request_changes verdict, contract-valid finding: AREV-007 not raised, execution verdict unaffected", () => {
  ...
  try {
    ...
    const id = baseResult(dir);
    newReview(dir, id,
      { recommendation: { verdict: "request_changes", notes: "the review disagrees with this execution" } },
      [{ finding_id: "AF-001", category: "acceptance-criteria-gap", severity: "major", status: "open", description: "d", suggestion: "s", requirement_ref: "REQ-001", implementation_claim: "the implementation misses the acceptance case", test_claim: "no test covers the acceptance case", owner: "Alice Chen" }],
    );
    const env = verifyResult(dir);
...
test("externally-observed: workflow attribution enforced via a stubbed gh", () => {
  ...
  try {
    ...
    const isolatedPolicy = JSON.parse(readFileSync(isolatedPolicyPath, "utf8")) as Record<string, unknown>;
    const binding = (isolatedPolicy["externally_observed_binding"] as Record<string, unknown>) ?? {};
    ...
    function writeStubGh(checkSuiteId: string, workflowPathForSuite: string, digest: string): void {
...
`;
    ...
    function verifyWithStub(): ExecEnvelope {
      const resultPath = join(dir, ".execution/D-001/EXECUTION-RESULT.json");
