...
export function getDesignOutputSetDigest(outputRoot) {
    ...
    if (lines.length === 0)
        return getSha256Hex("empty");
    return getSha256Hex(sortOrdinal(lines).join("\n"));
}
function testDesignOutputInventory(outputRoot, declaredOutputs) {
    const problems = [];
    ...
    for (const decl of declaredOutputs) {
        ...
        if (!actualFiles.has(relativePath.toLowerCase()))
            problems.push("missing declared output");
        else {
            const actualHash = getArtifactSha256(actualFiles.get(relativePath.toLowerCase()));
            if (String(decl["sha256"] ?? "").trim() === "" || actualHash !== String(decl["sha256"]).toLowerCase())
                problems.push("stale declared output digest");
        }
    }
    for (const relLower of actualFiles.keys()) {
        if (!declaredKeys.includes(relLower))
            problems.push("undeclared output");
    }
    return problems;
}
export function testDesignProviderWorkflow(acc, catalog, project, gate, orchestrationPolicy, decisionIds, handoffPolicy) {
    const policy = orchestrationPolicy["ui_delivery"] ?? {};
    ...
    if (!manifestExists && !requiredAtGate)
        return;
    if (gate === "Draft")
        return;
    if (!manifestExists) {
    ...
    for (const field of required) {
        ...
        if (typeof v !== "object" && String(v).trim() === "")
            structureProblems.push(field);
    }
    const inputs = manifest["inputs"] ?? [];
    if (inputs.length === 0)
        structureProblems.push("inputs");
    for (const input of inputs) {
        if (String(input["path"] ?? "").trim() === "" || String(input["sha256"] ?? "").trim() === "")
            structureProblems.push("input ref");
    }
    const generatedAt = String(manifest["generated_at"] ?? "");
    if (!/^\d{4}-\d{2}-\d{2}T/.test(generatedAt))
        structureProblems.push("generated_at");
    const inputPaths = inputs.map((i) => String(i["path"] ?? "").trim());
    if (!inputPaths.includes("PROJECT.md"))
        structureProblems.push("missing PROJECT.md input");
    if (existsSync(join(project, "DESIGN/BUILD-SPEC.md")) && !inputPaths.includes("DESIGN/BUILD-SPEC.md"))
        structureProblems.push("missing BUILD-SPEC input");
    for (const input of inputs) {
        const rel = String(input["path"] ?? "").trim();
        if (/^(source\/|\.\/source\/|\.\.|\/|\.\/)/.test(rel) && String(input["governed_justification"] ?? "").trim() === "")
            structureProblems.push("raw source input without justification");
    }
    // DPROV-003: freshness
    ...
    for (const input of inputs) {
        ...
        if (!/^[a-fA-F0-9]{64}$/.test(claimed) || getArtifactSha256(full) !== claimed.toLowerCase())
            freshnessProblems.push(rel);
    }
    ...
    if (freshnessProblems.length === 0) {
        ...
        if (!/^[a-fA-F0-9]{64}$/.test(declaredCombined) || recomputed !== declaredCombined.toLowerCase())
            freshnessProblems.push("combined_digest");
    }
    ...
    if (approvedExt === null)
        ...
        const extPayload = new Map();
        for (const ref of approvedExt["outgoing_artifacts"] ?? []) {
        ...
        for (const input of inputs) {
            ...
            if (!extPayload.has(pathKey) || extPayload.get(pathKey) !== hash)
                externalizationProblems.push(`payload ${String(input["path"] ?? "").trim()}`);
        }
    }
    ...
    if (reviewExists) {
        ...
        const rv = review;
        const preflight = rv["preflight"];
        acceptance = rv["acceptance"] ?? null;
        const declaredOutputs = rv["outputs"] ?? [];
        if (!preflight || !/^(passed|failed)$/.test(String(preflight["status"] ?? ""))) {
            ...
            if (freshnessProblems.length === 0) {
                ...
                if (!/^[a-fA-F0-9]{64}$/.test(declaredManifestDigest) || declaredManifestDigest.toLowerCase() !== declaredCombined.toLowerCase())
                    preflightProblems.push("stale manifest_digest");
            }
            const currentOutputs = getDesignOutputSetDigest(outputRoot);
            if (String(preflight["outputs_digest"] ?? "").trim() === "" || String(preflight["outputs_digest"]).toLowerCase() !== currentOutputs)
                preflightProblems.push("stale outputs");
            ...
            if (declaredOutputs.length === 0)
                preflightProblems.push("empty output inventory");
        }
        if (acceptance && String(acceptance["decision"] ?? "").trim() !== "" && (!preflight || String(preflight["status"]) !== "passed"))
            preflightProblems.push("review before preflight");
    }
    ...
    if (reviewExists) {
        if (acceptance && String(acceptance["decision"] ?? "").trim() !== "") {
            ...
            const acceptanceDecisions = policy["acceptance_decisions"] ?? [];
            const reviewerKinds = policy["reviewer_kinds"] ?? [];
            if (!acceptanceDecisions.includes(decision))
                authorityProblems.push("decision");
            const kind = String(acceptance["reviewer_kind"] ?? "");
            if (!reviewerKinds.includes(kind))
                authorityProblems.push("reviewer_kind");
            ...
            if (reviewer.trim() === "" || testGenericOwner(reviewer, ownerPolicy))
                authorityProblems.push("reviewer");
            if (decision === "accepted" && kind === "ai")
                authorityProblems.push("AI acceptance");
            ...
            if (!decisionRef || !(decisionIds ?? []).includes(decisionRef) || decider === null || testGenericOwner(decider, ownerPolicy))
                authorityProblems.push("decision_ref");
        }
        if (["Handoff", "Release"].includes(gate)) {
            if (!acceptance || String(acceptance["decision"] ?? "") !== "accepted")
                authorityProblems.push(`acceptance not accepted at ${gate}`);
            else if (String(acceptance["reviewer_kind"] ?? "") !== "human")
                authorityProblems.push(`acceptance not human at ${gate}`);
        }
    }
    // DPROV-007: change-control routing
    const changeControlProblems = [];
    let findings = [];
    ...
    if (existsSync(crRegistryPath)) {
        try {
            ...
            for (const change of crDoc.changes ?? []) {
                if (change.id)
                    registryIds.push(String(change.id));
                if (change.summary)
                    registrySummaries.push(String(change.summary));
            }
        }
        catch { }
    }
    let openBlockingFinding = false;
    const findingLenses = policy["finding_lenses"] ?? [];
    const findingImpacts = policy["finding_impacts"] ?? [];
    const findingStatuses = policy["finding_statuses"] ?? [];
    for (const finding of findings) {
        ...
        if (findingStatus === "resolved") {
            const decider = decisionRef && (decisionIds ?? []).includes(decisionRef) ? getDecisionDecider(project, decisionRef) : null;
            if (!decisionRef || !(decisionIds ?? []).includes(decisionRef) || decider === null || testGenericOwner(decider, ownerPolicy))
                changeControlProblems.push(`${findingId} resolution`);
        }
        const mustRoute = ["technical", "scope"].includes(impact) || ["technical", "scope"].includes(lens);
        if (mustRoute) {
            let routed = false;
            for (const id of registryIds) {
                if (id && summary.includes(id))
                    routed = true;
            }
            for (const entrySummary of registrySummaries) {
                if (entrySummary.includes(findingId))
                    routed = true;
            }
            if (!routed)
                changeControlProblems.push(`${findingId} not routed`);
            if (findingStatus !== "resolved")
                openBlockingFinding = true;
        }
    }
    if (reviewExists && acceptance && String(acceptance["decision"] ?? "") === "accepted" && openBlockingFinding)
        changeControlProblems.push("accepted with open blocking finding");
    if (structureProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider input manifest is incomplete: ${[...new Set(structureProblems)].join(", ")}`, { ruleId: "DPROV-002", artifact: "DESIGN/CLAUDE-DESIGN/INPUT-MANIFEST.json" });
    else
        addResult(acc, catalog, "PASS", "Design provider input manifest declares the required contract", { ruleId: "DPROV-002" });
    if (freshnessProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider manifest references or digests are invalid or stale: ${[...new Set(freshnessProblems)].join(", ")}`, { ruleId: "DPROV-003", artifact: "DESIGN/CLAUDE-DESIGN/INPUT-MANIFEST.json" });
    else
        addResult(acc, catalog, "PASS", "Design provider manifest references and digests are current", { ruleId: "DPROV-003" });
    if (externalizationProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider manifest externalization binding is invalid: ${[...new Set(externalizationProblems)].join(", ")}`, { ruleId: "DPROV-004", artifact: "DESIGN/CLAUDE-DESIGN/INPUT-MANIFEST.json", field: "externalization" });
    else
        addResult(acc, catalog, "PASS", "Design provider manifest cites a binding approved externalization entry", { ruleId: "DPROV-004" });
    if (preflightProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider preflight or output contract is invalid: ${[...new Set(preflightProblems)].join(", ")}`, { ruleId: "DPROV-005", artifact: "DESIGN/CLAUDE-DESIGN/REVIEW.json" });
    else if (!reviewExists)
        addResult(acc, catalog, "PASS", "No design provider review recorded yet (preflight not required before review exists)", { ruleId: "DPROV-005" });
    else
        addResult(acc, catalog, "PASS", "Design provider preflight and output placement are valid", { ruleId: "DPROV-005" });
    if (authorityProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider review authority is invalid: ${[...new Set(authorityProblems)].join(", ")}`, { ruleId: "DPROV-006", artifact: "DESIGN/CLAUDE-DESIGN/REVIEW.json" });
    else if (!reviewExists)
        addResult(acc, catalog, "PASS", "No design provider review recorded yet", { ruleId: "DPROV-006" });
    else
        addResult(acc, catalog, "PASS", "Design provider review carries valid Human acceptance evidence", { ruleId: "DPROV-006" });
    if (changeControlProblems.length)
        addResult(acc, catalog, "FAIL", `Design provider findings violate Change Control routing: ${[...new Set(changeControlProblems)].join(", ")}`, { ruleId: "DPROV-007", artifact: "DESIGN/CLAUDE-DESIGN/REVIEW.json" });
    else
        addResult(acc, catalog, "PASS", "Design provider findings are routed through Change Control", { ruleId: "DPROV-007" });
}
