...
This file is guidance. The machine-readable map is `pmo-config/context-map.json`; runtime policy is in `pmo-config/policy.json`.
...
| System design | `PROJECT.md`, `DESIGN/BUILD-SPEC.md`; also `DESIGN/SRS.md`, `DESIGN/DATA-DICTIONARY.md`, `DESIGN/ERD.puml` when `Spec depth: full` | Only for unresolved architecture/data/API questions | flow/wireframe, release docs |
...
| Mode-specific testability | `PROJECT.md`, `DESIGN/BUILD-SPEC.md` (Test Strategy table); also `TESTS/TEST-CASES.md` and `pmo-config/depth-policy.json` when `Spec depth: full` | Only rows already cited in the strategy | release docs, unrelated design |
| Data flow / journeys | `PROJECT.md`, `DESIGN/DATA-FLOW.md`, `DESIGN/BUILD-SPEC.md` (Data Model, State Machine) | Only for unresolved journey/state questions | delivery/release docs |
