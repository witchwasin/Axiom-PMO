...
function testCurrentDigestReference(project, reference, executionContract) {
    if (!reference)
        return false;
    ...
    if (!relative || isAbsolute(relative) || /^\.\.[\\/]/.test(relative))
        return false;
    if (executionContract && !/^\.execution\/[^/]+\/EXECUTION-CONTRACT\.json$/.test(relative.replace(/\\/g, "/")))
        return false;
...
export function testChangeControlRegistry(acc, catalog, project, gate, orchestrationPolicy, mode, executionPath, projectReqIds, decisionIds, handoffPolicy) {
    ...
    const blocking = [];
    const downstreamProblems = [];
    const cc = orchestrationPolicy["change_control"] ?? {};
    let blockingClassifications = cc["blocking_by_mode"]?.[mode];
    if (!blockingClassifications || blockingClassifications.length === 0) {
        blockingClassifications = cc["blocking_classifications"] ?? [];
    }
    const sources = cc["sources"] ?? [];
    const classifications = cc["classifications"] ?? [];
    const statuses = cc["statuses"] ?? [];
    const modeImpacts = cc["mode_impacts"] ?? [];
    ...
    for (const change of changes) {
        ...
        const missing = required.filter((f) => change[f] === undefined || String(change[f] ?? "").trim() === "");
        if (!/^CR-\d{3,}$/.test(id) || missing.length > 0) {
            structureProblems.push(id || "unnamed entry");
            continue;
        }
        if (!sources.includes(change.source ?? ""))
            structureProblems.push(`${id} source`);
        if (!classifications.includes(change.classification ?? ""))
            structureProblems.push(`${id} classification`);
        if (!statuses.includes(change.status ?? ""))
            structureProblems.push(`${id} status`);
        if (!modeImpacts.includes(change.mode_impact ?? ""))
            structureProblems.push(`${id} mode impact`);
        for (const req of change.affected_requirements ?? []) {
            if (!projectReqIds.includes(req))
                structureProblems.push(`${id} requirement ref`);
        }
        ...
        if (change.status === "implemented" && hasDownstreamImpact) {
            ...
            if (artifactRefs.length === 0 || artifactRefs.some((a) => !testCurrentDigestReference(project, a, false)))
                invalid = true;
            if (executionPath === "governed_ai_execution") {
                const contractRefs = validation?.execution_contracts ?? [];
                if (contractRefs.length === 0 || contractRefs.some((c) => !testCurrentDigestReference(project, c, true)))
                    invalid = true;
            }
            if (invalid)
                downstreamProblems.push(id);
        }
