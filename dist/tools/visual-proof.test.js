...
import { mkdtempSync, mkdirSync, cpSync, rmSync, writeFileSync, readFileSync, existsSync, appendFileSync, } from "node:fs";
...
const proofPolicy = (cfg.handoffPolicy["visual_proof"] ?? {});
const DIRECTION = [
...
function addVisualProofArtifacts(project) {
    ...
    for (const capture of proofPolicy["captures"] ?? []) {
    ...
    const handoffReview = JSON.parse(readFileSync(handoffReviewPath, "utf8"));
    const reviewInputs = handoffReview["review_inputs"];
...
function newVisualProofReview(project) {
    const captures = (proofPolicy["captures"] ?? []).map((capture) => {
    ...
    const rubric = (proofPolicy["rubric"] ?? []).map((item) => ({
        id: String(item["id"]),
        status: "reviewed",
...
function newVisualProofProject(workRoot) {
    const project = newTestProject(workRoot);
    addVisualProofArtifacts(project);
    newVisualProofReview(project);
    return project;
}
function readReview(project) {
    return JSON.parse(readFileSync(join(project, String(proofPolicy["artifact"])), "utf8"));
}
function writeReview(project, review) {
    writeUtf8(join(project, String(proofPolicy["artifact"])), JSON.stringify(review) + "\n");
}
function ruleHits(diagnostics, ruleId, level = "") {
    return diagnostics.filter((d) => d.rule_id === ruleId && (level === "" || d.level === level));
}
function chainFailed(diagnostics) {
    return diagnostics.some((d) => d.level === "FAIL");
}
function failureSummary(diagnostics) {
...
test("visual proof: policy requires human reviewer attestation only", () => {
    const reviewerKinds = (proofPolicy["reviewer_kinds"] ?? []).map(String);
    assert.ok(reviewerKinds.length === 1 && reviewerKinds[0] === "human", `Visual Proof policy requires human reviewer attestation only (reviewer_kinds=${reviewerKinds.join(", ")})`);
});
...
test("visual proof: failure cases", () => {
    ...
    try {
        ...
        const desktop = proofPolicy["captures"].find((c) => String(c["id"]) === "desktop");
        ...
        const wrongPathDesktop = wrongPathReview["captures"].find((c) => c["id"] === "desktop");
