...
function runGit(repoRoot: string, args: string[]): { out: string; code: number | null } {
  const r = spawnSync("git", ["-C", repoRoot, ...args], { encoding: "utf8" });
  return { out: r.stdout ?? "", code: r.status };
}

export function testExecutionRefResolvable(repoRoot: string, ref: string): boolean {
  const { out, code } = runGit(repoRoot, ["rev-parse", "--verify", "--quiet", `${ref}^{commit}`]);
  return code === 0 && out.trim() !== "";
}

export function resolveExecutionCommitSha(repoRoot: string, ref: string): string | null {
  const { out, code } = runGit(repoRoot, ["rev-parse", "--verify", "--quiet", `${ref}^{commit}`]);
...
export function testExecutionAncestry(repoRoot: string, ancestor: string, descendant: string): boolean {
  const { code } = runGit(repoRoot, ["merge-base", "--is-ancestor", ancestor, descendant]);
  return code === 0;
}

export function getExecutionCommitRange(repoRoot: string, baseRef: string, headRef: string): string[] | null {
...
export function getExecutionChangedFiles(repoRoot: string, baseRef: string, headRef: string): ReturnType<typeof getScopeDiffChangedFiles> {
  return getScopeDiffChangedFiles(repoRoot, baseRef, headRef);
}

export function testExecutionCommitOnRemote(repoRoot: string, commitSha: string): boolean | null {
  ...
  if (containing.code !== 0) return false;
...
export interface ExecutionGitObservation {
  ok: boolean;
  ...
  headOnRemote?: boolean | null;
}

export function getExecutionGitObservation(repoRoot: string, baseRef: string, headRef: string): ExecutionGitObservation {
