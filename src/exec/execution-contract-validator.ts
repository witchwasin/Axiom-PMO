...
import {
  readExecutionContract, readExecutionResult, readExecutionContractPolicy, resolveTestEvidenceEntries,
  resolveDecisionRecord, testDecisionAuthorityBinding,
  type ContractRead, type ResultRead, type TestEvidenceEntry, type DecisionRecord,
} from "./execution-contract-schema.js";
import { getExecutionGitObservation, type ExecutionGitObservation } from "./execution-contract-git.js";
...
export interface VerificationVerdict {
  ...
  changed_files_out_of_scope: string[];
  unverified_required_tests: string[];
  authority_violations: string[];
  verdict: string;
}

export function invokeExecutionContractVerification(
  ...
  if (!contractPath) contractPath = join(dirname(resolvedResultPath), "EXECUTION-CONTRACT.json");
  ...
  verdict.work_item_id = String((contract.document as Record<string, unknown>)["work_item_id"] ?? "");
  verdict.base_sha = String((contract.document as Record<string, unknown>)["base_sha"] ?? "");
  ...
  const contractDoc = contract.document as Record<string, unknown>;
  ...
  const contractRequirements = ((contractDoc["requirement_refs"] as unknown[]) ?? []).map((r) => String(r));
  if (doc["requirement_refs"] !== undefined) {
    for (const req of (doc["requirement_refs"] as unknown[]) ?? []) {
  ...
  const exemptRegexes: RegExp[] = ((policy["verification_exempt_paths"] as Array<Record<string, unknown>>) ?? []).map((e) => new RegExp(convertToScopeGlobRegex(String(e["pattern"]))));
  const testExemptPath = (path: string) => exemptRegexes.some((rx) => rx.test(path));

  const observedPaths: string[] = [];
  for (const change of observation.changes!) {
    const path = change.path;
    if (!testExemptPath(path)) observedPaths.push(path);
    if (change.oldPath && !testExemptPath(change.oldPath)) observedPaths.push(change.oldPath);
  }
  verdict.changed_files_observed = observedPaths;

  let decisionLogRelPath: string | null = null;
  ...
  if (doc["changed_files"] !== undefined) {
    const claimedPaths = ((doc["changed_files"] as unknown[]) ?? []).map((p) => String(p));
  ...
  // 5. Scope (EXEC-004)
  const allowedRegexes = ((contractDoc["allowed_paths"] as string[]) ?? []).map((p) => new RegExp(convertToScopeGlobRegex(p)));
  const prohibitedRegexes: RegExp[] = ((contractDoc["prohibited_paths"] as string[]) ?? []).map((p) => new RegExp(convertToScopeGlobRegex(p)));
  ...
  const evidenceProvenance = (policy["evidence_provenance"] as Record<string, unknown>) ?? {};
  const satisfyingTiers: string[] = [];
  for (const [tier, spec] of Object.entries(evidenceProvenance)) {
    if (tier === "_note") continue;
    if ((spec as Record<string, unknown>)["satisfies_required_test"] === true) satisfyingTiers.push(tier);
  }

  const requiredTestSatisfaction = (policy["required_test_satisfaction"] as Record<string, unknown>) ?? {};
  const humanVouchType = String(requiredTestSatisfaction["human_vouch_claim_type"] ?? "test-evidence-accepted");

  function resolveEvidenceVouch(
    claims: Array<Record<string, unknown>>, vouchType: string, testName: string,
    entry: TestEvidenceEntry, evidenceDigest: string, workItemId: string,
    contractDigest: string, projectPath: string, decisionLogRel: string | null, observedPaths: string[],
  ): string | null {
    const candidates = claims.filter((c) => String(c["type"] ?? "") === vouchType);
    if (candidates.length === 0) return `no ${vouchType} claim is present`;

    let lastReason: string | null = null;
    for (const claim of candidates) {
      if (String(claim["actor"] ?? "") !== "human") { lastReason = `the vouch is from actor '${claim["actor"]}', and only a human may accept test evidence`; continue; }
      const claimTest = String(claim["test_name"] ?? "");
      if (!claimTest.trim()) { lastReason = "the vouch names no test_name, so it cannot be tied to any particular required test"; continue; }
      if (claimTest !== testName) { lastReason = `the only vouch present is for test '${claimTest}', not '${testName}'`; continue; }

      const claimDigest = String(claim["evidence_sha256"] ?? "").trim().toLowerCase();
      if (!claimDigest.trim()) { lastReason = "the vouch names no evidence_sha256, so it does not identify which artifact was accepted"; continue; }
      if (claimDigest !== evidenceDigest) { lastReason = `the vouch accepts artifact digest ${claimDigest}, but the evidence actually presented digests to ${evidenceDigest}`; continue; }

      if (claim["evidence_type"] !== undefined && String(claim["evidence_type"] ?? "").trim() !== "" && String(claim["evidence_type"]) !== entry.type) {
        lastReason = `the vouch is for '${claim["evidence_type"]}' evidence, but this entry is '${entry.type}'`; continue;
      }
      if (claim["work_item_id"] !== undefined && String(claim["work_item_id"] ?? "").trim() !== "" && String(claim["work_item_id"]) !== workItemId) {
        lastReason = `the vouch is bound to work item '${claim["work_item_id"]}', not '${workItemId}'`; continue;
      }
      if (claim["contract_sha256"] !== undefined && String(claim["contract_sha256"] ?? "").trim() !== "" && String(claim["contract_sha256"]).toLowerCase() !== contractDigest) {
        lastReason = "the vouch is bound to a different contract digest than the one being verified"; continue;
      }

      const vouchRef = String(claim["decision_ref"] ?? "");
      if (!vouchRef.trim()) { lastReason = "the vouch cites no decision record"; continue; }
      const resolved = resolveDecisionRecord(projectPath, vouchRef);
      if (!resolved.found) { lastReason = `the vouch cites decision record '${vouchRef}', which could not be resolved: ${resolved.reason}`; continue; }
      if (decisionLogRel && observedPaths.includes(decisionLogRel)) {
        lastReason = `the vouch cites '${vouchRef}', but decision-log.md was itself changed within the commit range under verification`; continue;
      }
      ...
      if (bindingProblem) { lastReason = `decision record '${vouchRef}' does not authorize this: ${bindingProblem}`; continue; }
  ...
  if (contractDoc["required_tests"] !== undefined) {
    for (const required of (contractDoc["required_tests"] as unknown[]) ?? []) {
      ...
      if (satisfyingTiers.includes(match.provenance)) continue;

      const claims = ((doc["authority_claims"] as Array<Record<string, unknown>>) ?? []);
  ...
  const authority = (contractDoc["git_authority"] as Record<string, unknown>) ?? {};
  const testGrantedAction = (action: string) => authority[action] === true;
  ...
  if (doc["git_actions_performed"] !== undefined) {
    for (const action of (doc["git_actions_performed"] as unknown[]) ?? []) {
      const actionText = String(action);
      if (!actionText.trim()) continue;
      if (!testGrantedAction(actionText)) {
        if (!violations.includes(actionText)) violations.push(actionText);
  ...
  if (doc["authority_claims"] !== undefined) {
    const actorAuthority = (policy["actor_authority"] as Record<string, unknown>) ?? {};
    const authorityClaimTypes = (policy["authority_claim_types"] as Record<string, unknown>) ?? {};
    for (const claim of (doc["authority_claims"] as Array<Record<string, unknown>>) ?? []) {
      const claimType = String(claim["type"] ?? "");
      const actor = String(claim["actor"] ?? "");
      if (!claimType.trim()) continue;

      const actorPolicy = actorAuthority[actor] as Record<string, unknown> | undefined;
      if (!actorPolicy) {
      ...
      const mayGrant = ((actorPolicy["may_grant"] as unknown[]) ?? []).map((g) => String(g));
      ...
      const typePolicy = authorityClaimTypes[claimType] as Record<string, unknown> | undefined;
  ...
  if (!preflight) {
    ...
    testAdversarialReviewEvidence(
      acc, catalog, projectPath, contractPath, contract,
      effectiveModeForArev, doc, verdict.base_sha!, verdict.head_sha!, verdict.work_item_id!,
      frameworkRoot, gitRepoRoot, observedPaths, decisionLogRelPath, projectReqIds,
    );
  }
