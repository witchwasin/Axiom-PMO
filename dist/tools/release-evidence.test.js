...
const referenceTypesConfig = cfg.referenceTypesConfig;
...
function writeStandardReleaseProject(dir, includeEvidenceFile, evidencePath = "tests/evidence/report.xml", defaultMode = "Standard") {
...
function runReleaseValidate(project, mode, base, head) {
...
function testEvidenceRows(diagnostics, level) {
...
test("release evidence: fresh evidence inside the verified range", () => {
    ...
    try {
        ...
        assert.equal(testEvidenceRows(r.diagnostics, "WARN").length + testEvidenceRows(r.diagnostics, "FAIL").length, 0, "fresh-in-range: no TEST-EVIDENCE-003 WARN/FAIL row");
...
test("release evidence: stale evidence is WARN-blocking at Standard", () => {
    ...
    try {
        ...
        assert.match(warns[0].message, /not changed within the release's verified commit range/, "stale-standard: message names the git-ground-truth defect");
...
test("release evidence: untracked evidence is out of scope entirely", () => {
    ...
    try {
        ...
        writeFixtureFile(dir, "tests/evidence/report.xml", '<testsuite name="happy" tests="1" failures="0"/>');
        const r = runReleaseValidate(dir, "Standard", base, head);
        assert.equal(testEvidenceRows(r.diagnostics, "WARN").length + testEvidenceRows(r.diagnostics, "FAIL").length, 0, "untracked: no TEST-EVIDENCE-003 WARN/FAIL row (out of scope)");
