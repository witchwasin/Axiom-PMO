...
export function hookScopeAdvisory(projectPath: string | null, payloadText: string): AdvisoryResult {
  const silent: AdvisoryResult = { output: "", exitCode: 0 };
  try {
    if (!payloadText?.trim()) return silent;
    let payload: Record<string, unknown>;
    try { payload = JSON.parse(payloadText); } catch { return silent; }
    if (!payload) return silent;

    const toolInput = payload["tool_input"] as Record<string, unknown> | undefined;
    if (!toolInput) return silent;

    const candidatePaths: string[] = [];
    for (const field of ["file_path", "path", "notebook_path"]) {
      const value = toolInput[field];
      if (value && typeof value === "string") candidatePaths.push(value);
    }
    if (candidatePaths.length === 0) return silent;

    let project = projectPath ?? (payload["cwd"] as string | undefined) ?? process.cwd();
    if (!existsSync(project)) return silent;
    const resolvedProject = resolve(project);

    const optInPath = join(resolvedProject, ".axiom/hooks.json");
    if (!existsSync(optInPath)) return silent;
    let optIn: Record<string, unknown>;
    try { optIn = JSON.parse(readFileSync(optInPath, "utf8")); } catch { return silent; }
    if (!optIn || optIn["scope_advisory"] !== true) return silent;

    const declaration = readScopeDeclaration(resolvedProject);
    if (!declaration.present || !declaration.valid) return silent;
    ...
    for (const candidate of candidatePaths) {
      ...
      if (verdict.verdict === "out_of_scope") findings.push(relative);
    }
