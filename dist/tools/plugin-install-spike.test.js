...
import { mkdtempSync, mkdirSync, cpSync, rmSync, writeFileSync, readFileSync, readdirSync, statSync, existsSync, chmodSync, } from "node:fs";
...
function newSimulatedPluginInstall(root) {
...
test("plugin install spike: framework-root vs project-root separation", () => {
    ...
    try {
        ...
        assert.ok(doctorText.includes("validate") || doctorText.includes("axiom.mjs"), "...and the diagnostic redirects to the user-facing command");
        ...
        const missingVerdict = String(verifyMissing.envelope["execution_verification"]["verdict"]);
        assert.ok(/missing|EXEC-/i.test(missingVerdict) || verifyMissing.exitCode !== 0, `verify refuses a missing result cleanly rather than crashing: verdict=${missingVerdict}`);
