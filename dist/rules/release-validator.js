...
import { getTableRowsAfterHeading, getTableLinesAfterHeading } from "../markdown/table-parser.js";
...
import { getReleaseRegistry, testTestSummary, testRtmTraceability } from "./rtm-validator.js";
export function testRaidBlocker(acc, catalog, project, gate) {
    const raidPath = join(project, "RAID-log.md");
    if (!existsSync(raidPath))
        return;
...
function testReviewRow(acc, catalog, ctx, releaseText, reviewType, ruleId, approvalMode, decisionIds, registry) {
    ...
    if (row["Status"] !== "approved")
        invalid.push("status");
    if (testPlaceholderValue(row["Reviewer"] ?? ""))
        invalid.push("reviewer");
    if (testPlaceholderValue(row["Role"] ?? ""))
        invalid.push("role");
    if (testPlaceholderValue(row["Date"] ?? "") || !testDateValue(row["Date"] ?? ""))
        invalid.push("date");
    ...
    const allowedRoles = ctx.policy["approval_roles"]?.[`${reviewType} Approved`];
...
export function testReleaseArtifact(acc, catalog, ctx, project, mode, gate, deliveryIds, decisionIds) {
    let releaseText = "";
    const releasePath = join(project, "RELEASE.md");
    let releaseRegistry = { releaseId: null, testIds: [], testRows: [] };
    ...
    if (waiver) {
        const waiverRule = ctx.policy["rollback_waiver"] ?? {};
        const waiverAllowedModes = waiverRule["allowed_modes"] ?? [];
        const waiverAllowedTypes = waiverRule["allowed_change_types"] ?? [];
        const waiverInvalid = [];
        if (!waiverAllowedModes.includes(mode))
            waiverInvalid.push(`mode ${mode} is not allowed to waive rollback`);
        if (!waiverAllowedTypes.includes(waiver.changeType))
            waiverInvalid.push(`change_type '${waiver.changeType}' is not on the waiver allowlist`);
        if (testPlaceholderValue(waiver.reason))
            waiverInvalid.push("reason is missing");
        if (testPlaceholderValue(waiver.approver))
            waiverInvalid.push("approver is missing");
        ...
        if (rollbackSectionLines.length >= 3) {
            for (const line of rollbackSectionLines.slice(2)) {
                ...
                rollbackDataRows++;
                if (cells.slice(0, 5).some((c) => testPlaceholderValue(c)))
                    badRollbackRows.push(line);
            }
        }
    ...
    if (mode !== "Lite") {
        ...
        if (releaseRegistry.testRows.length === 0) {
            if (testWaiver) {
                const testWaiverRule = ctx.policy["test_waiver"] ?? {};
                const testWaiverAllowedModes = testWaiverRule["allowed_modes"] ?? [];
                const testWaiverInvalid = [];
                if (!testWaiverAllowedModes.includes(mode))
                    testWaiverInvalid.push(`mode ${mode} is not allowed to waive testing`);
                if (testPlaceholderValue(testWaiver.reason))
                    testWaiverInvalid.push("reason is missing");
                if (testPlaceholderValue(testWaiver.approvedBy))
                    testWaiverInvalid.push("approved_by is missing");
                if (testPlaceholderValue(testWaiver.evidence)) {
                    ...
                    if (!testWaiverRef.type || !testWaiverRef.resolved)
                        testWaiverInvalid.push("evidence does not resolve");
                }
...
export function testReleaseScopeCompletion(acc, catalog, ctx, workItems, releaseText, mode, gate, decisionIds, releaseRegistry) {
    if (!(gate === "Release" && workItems && workItems.length > 0))
        return;
    let releaseScopeRows = [];
    if (releaseText)
        releaseScopeRows = getTableRowsAfterHeading(releaseText, "^##\\s+Release Scope");
...
export function testStrictReleaseGuardrails(acc, catalog, ctx, policyEnums, sourceRefRegex, project, mode, gate, projectReqIds, deliveryIds, decisionIds, releaseRegistry, projectSourceIds) {
    if (!(mode === "Strict" && gate === "Release"))
        return;
    const strictMissing = [];
    if (!existsSync(join(project, "RTM.json")))
        strictMissing.push("RTM.json");
    if (!existsSync(join(project, "RAID-log.md")))
        strictMissing.push("RAID-log.md");
    if (!existsSync(join(project, "decision-log.md")))
        strictMissing.push("decision-log.md");
