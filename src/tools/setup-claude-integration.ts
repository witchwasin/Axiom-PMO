...
export function setupClaudeIntegration(
  ...
  for (const probe of [
    ...
    if (existsSync(join(project, probe.path))) neighbours.push(`${probe.label} (${probe.path})`);
  }

  const body = getAxiomCanonicalBody("1")!;

  if (uninstall) {
    if (!state.exists) return { output: `Nothing to remove: ${file} does not exist.\n`, exitCode: EXIT_OK };
    const removal = removeAxiomBlock(state.text, state.newline, force);
    if (removal.action === "absent") return { output: `Nothing to remove: ${file} has no Axiom-PMO block.\n`, exitCode: EXIT_OK };
  ...
  try {
    if (state.exists) backup = newAxiomBackup(targetPath);
