...
if (onlyTs.length === 0 && onlyPs.length === 0) {
    ...
    for (const [k, v] of onlyTs)
        console.log(`  only TS: ${k} x${v}`);
    for (const [k, v] of onlyPs)
        console.log(`  only PS: ${k} x${v}`);
    process.exit(1);
}
