...
function verdict(path: string, include: string[], exclude: string[] = []) {
  return resolveScopeVerdict(
    path,
    include.map((p) => new RegExp(convertToScopeGlobRegex(p))),
    exclude.map((p) => new RegExp(convertToScopeGlobRegex(p))),
    [],
  ).verdict;
}
