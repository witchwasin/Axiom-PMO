...
import {
  mkdtempSync, mkdirSync, cpSync, rmSync, readFileSync, writeFileSync, readdirSync, statSync,
...
function assertClean(results: Diagnostic[], name: string): void {
...
function restore(active: string, rels: string[]): void {
  for (const rel of rels) cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS", rel), join(active, rel));
}
...
test("M4-M6: contract mutations against OPTIONAL-TRACKS", () => {
  ...
  try {
    ...
    let extDoc = readJson(extPath) as { entries: Array<Record<string, unknown>> };
    ...
    extDoc = readJson(extPath) as typeof extDoc;
    extDoc.entries[0] = {
      ...extDoc.entries[0],
    ...
    let manifestDoc = readJson(manifestPath) as { inputs: Array<Record<string, unknown>>; externalization?: string };
    ...
    (reviewDoc["acceptance"] as Record<string, unknown>)["reviewer_kind"] = "ai";
    (reviewDoc["acceptance"] as Record<string, unknown>)["decision"] = "accepted";
    (reviewDoc["acceptance"] as Record<string, unknown>)["decision_ref"] = "DEC-002";
    writeJson(reviewPath, reviewDoc);
    result = invoke(tempRepo, active, "Standard", "Design");
    ...
    reviewDoc = readJson(reviewPath) as Record<string, unknown>;
    (reviewDoc["acceptance"] as Record<string, unknown>)["reviewer_kind"] = "human";
    (reviewDoc["acceptance"] as Record<string, unknown>)["decision"] = "accepted";
    writeJson(reviewPath, reviewDoc);
    ...
    let provDoc = readJson(provenancePath) as { claims: Array<Record<string, unknown>>; fallback_used?: boolean; provider_available?: boolean; provider_used?: string; retrieved_at?: string };
    provDoc.claims[0]!["sources"] = [];
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    assertRule(result, "RESEARCH-003", "claim without a source");

    provDoc = readJson(provenancePath) as typeof provDoc;
    ...
    writeFileSync(
      researchReport,
      readFileSync(researchReport, "utf8").replace(
        "| CP-001 | Defer multi-warehouse stock | scope | yes | accepted | Demo PO | DEC-999 |",
        "| CP-001 | Defer multi-warehouse stock | scope | yes | proposed | Demo PO | |",
      ),
      "utf8",
    );
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    provDoc = readJson(provenancePath) as typeof provDoc & { externalization?: string };
    provDoc.fallback_used = false;
    provDoc.provider_available = true;
    (provDoc as Record<string, unknown>)["externalization"] = "EXT-999";
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    const orchestrationPolicy = readJson(orchestrationPolicyPath) as { externalization: Record<string, unknown> };
    ...
    reviewDoc = readJson(reviewPath) as Record<string, unknown>;
    (reviewDoc["preflight"] as Record<string, unknown>)["manifest_digest"] = "0".repeat(64);
    writeJson(reviewPath, reviewDoc);
    result = invoke(tempRepo, active, "Standard", "Design");
    ...
    writeFileSync(
      ...
      readFileSync(researchReport, "utf8").replace(
        ...
        "| CP-001 | Defer multi-warehouse stock; demo covers a single site | scope | yes | rejected | Demo PO | |",
      ),
      "utf8",
    );
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS/RESEARCH/PROVENANCE.json"), provenancePath);
    provDoc = readJson(provenancePath) as typeof provDoc;
    delete (provDoc.claims[0]!["sources"] as Array<Record<string, unknown>>)[0]!["primary"];
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS/RESEARCH/PROVENANCE.json"), provenancePath);
    provDoc = readJson(provenancePath) as typeof provDoc;
    (provDoc.claims[0]!["sources"] as Array<Record<string, unknown>>)[0]!["verification"] = "invented";
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS/RESEARCH/PROVENANCE.json"), provenancePath);
    provDoc = readJson(provenancePath) as typeof provDoc;
    (provDoc.claims[0]!["sources"] as Array<Record<string, unknown>>)[0]!["date"] = "2026-06-30";
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS/RESEARCH/PROVENANCE.json"), provenancePath);
    provDoc = readJson(provenancePath) as typeof provDoc;
    delete (provDoc.claims[0]!["sources"] as Array<Record<string, unknown>>)[0]!["date"];
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    cpSync(join(REPO_ROOT, "examples/OPTIONAL-TRACKS/RESEARCH/PROVENANCE.json"), provenancePath);
    provDoc = readJson(provenancePath) as typeof provDoc;
    delete (provDoc as Record<string, unknown>)["retrieved_at"];
    writeJson(provenancePath, provDoc);
    result = invoke(tempRepo, active, "Standard", "Scope");
    ...
    extDoc = readJson(extPath) as typeof extDoc;
    (extDoc.entries[0]!["outgoing_artifacts"] as Array<Record<string, unknown>>).push({ path: "assets/logo.png", sha256: pngHash });
    writeJson(extPath, extDoc);
    result = invoke(tempRepo, active, "Standard", "Design");
    ...
    newProject(
      tempRepo, "E2E-TRACKS", "Standard", "development_handoff", "guided", "standard", "feyman",
      "claude_design", "none", "normal feature", "PM", genOut, false, "internal", 14,
    );
