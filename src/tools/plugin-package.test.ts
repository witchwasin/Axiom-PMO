...
import {
  mkdtempSync, mkdirSync, cpSync, rmSync, writeFileSync, readFileSync, readdirSync, statSync, existsSync,
} from "node:fs";
...
test("plugin package: the manifests", () => {
  ...
  assert.match(
    String(plugin["name"]),
    /^[a-z][a-z0-9]*(-[a-z0-9]+)*$/,
    `plugin name is kebab-case as the manifest reference requires (name=${plugin["name"]})`,
  );
  ...
  assert.equal(
    String(plugin["version"]), repoVersion,
    `plugin version equals the release VERSION (plugin=${plugin["version"]} VERSION=${repoVersion} -- the plugin's declared version is the identity Claude Code caches against; a mismatch ships bytes under a version that is not theirs)`,
  );
  ...
  assert.ok(
    !("skills" in plugin),
    "plugin manifest declares no component path overrides for skills (plugin.json has no 'skills' field in the documented schema; declaring one would be silently ignored)",
  );
});
...
test("plugin package: the generated skills mirror", () => {
  ...
  const dirNames = (root: string): string[] =>
    readdirSync(root).filter((e) => statSync(join(root, e)).isDirectory()).sort();
  const sourceSkills = dirNames(join(REPO_ROOT, ".claude/skills"));
  const mirrorSkills = dirNames(mirrorRoot);
  assert.equal(
    mirrorSkills.join(","), sourceSkills.join(","),
    `the mirror carries every skill and no others (source=${sourceSkills.join(",")} mirror=${mirrorSkills.join(",")})`,
  );
  ...
  const manifestSkills = ((manifest["active_skills"] as Array<Record<string, unknown>>) ?? [])
  ...
  for (const skill of mirrorSkills) {
    ...
    assert.match(
      text,
      /^---\s*\r?\n[\s\S]*?^name:\s*\S+[\s\S]*?^description:\s*\S+[\s\S]*?^---\s*$/m,
      `packaged skill '${skill}' declares name and description frontmatter`,
    );
  }
});
...
test("plugin package: ownership registry", () => {
  ...
  assert.ok(
    KNOWN_BODY_DIGESTS.includes(canonicalDigest),
    `the current canonical body's digest is in the frozen registry (computed=${canonicalDigest} -- if the body was edited, append this digest to KNOWN_BODY_DIGESTS rather than replacing the existing entry)`,
  );
  assert.ok(
    KNOWN_BODY_DIGESTS.length >= 1,
    "the registry keeps every historical digest, never just the current one",
  );
...
test("plugin package: the drift gate", () => {
  ...
  try {
    ...
    buildPluginPackage(copy, false);
    assert.ok(
      !existsSync(join(copy, "skills/SHOULD-NOT-BE-PACKAGED.md")),
      "the generator packages only .claude/skills, nothing from the repository root",
    );
  } finally {
...
test("plugin package: FRAMEWORK-001 guard outside a checkout", () => {
  ...
  try {
    ...
    assert.equal(
      testFrameworkCheckout(REPO_ROOT), true,
      "the guard recognises this repository as a real checkout",
    );
  } finally {
...
test("plugin package: release-check catches plugin-manifest version drift", () => {
  ...
  try {
    ...
    assert.match(
      result.output, /version drift/i,
      "the release check reports version drift when plugin.json disagrees with VERSION",
    );
    assert.match(
      result.output, /PLUGIN=9\.9\.9/,
      "...and names the plugin manifest specifically, not just VERSION/CHANGELOG/config",
    );
  } finally {
