...
export function designProviderDigest(repoRoot: string, projectPath: string): { output: string; exitCode: number } {
  const project = resolve(projectPath);
  const cfg = importPmoConfig(repoRoot);
  const orchestrationPolicy = cfg.orchestrationPolicy;
  const manifestPath = join(project, String(((orchestrationPolicy["ui_delivery"] as Record<string, unknown>) ?? {})["input_manifest"] ?? "DESIGN/CLAUDE-DESIGN/INPUT-MANIFEST.json"));
  if (!existsSync(manifestPath)) return { output: `No input manifest found: ${manifestPath}\n`, exitCode: 1 };

  const manifest = JSON.parse(readFileSync(manifestPath, "utf8")) as Record<string, unknown>;
  const inputs = (manifest["inputs"] as Array<Record<string, unknown>>) ?? [];

  const lines: string[] = [];
  const currentInputs: Array<Record<string, unknown>> = [];
  ...
  const outputRoot = join(project, String(((orchestrationPolicy["ui_delivery"] as Record<string, unknown>) ?? {})["output_root"] ?? "DESIGN/CLAUDE-DESIGN/OUTPUT"));
...
export function handoffDigest(repoRoot: string, projectPath: string, which = "Both"): { output: string; exitCode: number } {
  const project = resolve(projectPath);
  ...
  if (!sourceDigest) return { output: "PROJECT.md has no Source Snapshot or Source Inventory table to digest.\n", exitCode: 1 };
  const inputDigest = getReviewInputDigest(project, handoffPolicy);

  if (which === "Source") return { output: sourceDigest + "\n", exitCode: 0 };
  if (which === "ReviewInputs") return { output: (inputDigest ?? "") + "\n", exitCode: 0 };
  return { output: `source_snapshot.digest : ${sourceDigest}\nreview_inputs.digest   : ${inputDigest}\n`, exitCode: 0 };
}

export function visualProofDigest(repoRoot: string, projectPath: string): { output: string; exitCode: number } {
  const project = resolve(projectPath);
  const cfg = importPmoConfig(repoRoot);
  const proof = (cfg.handoffPolicy["visual_proof"] as Record<string, unknown>) ?? null;
  if (!proof) return { output: "handoff-policy.json has no visual_proof policy.\n", exitCode: 1 };
  if (!testVisualProofActivated(project, proof)) return { output: "Visual Proof is inactive: the project does not contain all configured creative artifacts.\n", exitCode: 1 };
