...
export function runExecutionCommand(projectPath, workItemId, name, command, workingDirectory = ".", contractPath = null) {
    ...
    if (!contract.present)
        return { output: `RUN FAILED: No execution contract at ${contractP}. A run record must be bound to an exported contract's digest.\n`, exitCode: 1 };
    if (!contract.valid)
        return { output: `RUN FAILED: Execution contract is invalid: ${contract.error}\n`, exitCode: 1 };
    const sidecarPath = contractP + ".sha256";
    if (!existsSync(sidecarPath))
        return { output: `RUN FAILED: No digest sidecar at ${sidecarPath}. Export the contract with axiom export before running evidence against it.\n`, exitCode: 1 };
    const sidecarDigest = readFileSync(sidecarPath, "utf8").trim().toLowerCase();
    if (sidecarDigest !== contract.digest)
        return { output: "RUN FAILED: The contract's digest does not match its sidecar -- it was modified after export. Re-export before recording evidence against it.\n", exitCode: 1 };
    // containment
    ...
    if (!existsSync(cwdFull))
        return { output: `RUN FAILED: -WorkingDirectory does not exist: ${workingDirectory}\n`, exitCode: 1 };
