...
function loadJson(repo, rel) {
    ...
    try {
        ...
        if (raw.charCodeAt(0) === 0xfeff)
            raw = raw.slice(1); // strip BOM (CR-019)
...
export function runPmoDoctor(repo, skillRootOverride = "", templateRootOverride = "") {
    ...
    for (const name of ["LITE-BUGFIX", "STANDARD-FEATURE", "STRICT-HIGH-RISK", "P01-DEMO"]) {
        ...
        if (existsSync(join(repo, examplePath)) && statSync(join(repo, examplePath)).isDirectory())
            add("PASS", `Found ${examplePath}`, "DOCTOR-EXAMPLE");
        else
            add("FAIL", `Missing ${examplePath}`, "DOCTOR-EXAMPLE");
    }
    const activeSkills = (skillManifest?.["active_skills"] ?? []).map((s) => String(s["id"]));
    if (activeSkills.length === 0)
        add("FAIL", "No active skills declared in pmo-config/skill-manifest.json", "DOCTOR-001");
    ...
    if (missingActive.length === 0 && extraActive.length === 0)
        add("PASS", "Active skill runtime contains exactly 7 skills", "DOCTOR-001");
    else {
        if (missingActive.length > 0)
            add("FAIL", `Missing active skills: ${missingActive.join(", ")}`, "DOCTOR-001");
        if (extraActive.length > 0)
            add("FAIL", `Unexpected active skills: ${extraActive.join(", ")}`, "DOCTOR-001");
    }
    // Test-SkillFrontmatter (DOCTOR-SKILL-001)
    {
        ...
        for (const skill of activeSkills) {
            ...
            const fmMatch = /^---\s*\r?\n([\s\S]*?)\r?\n---/.exec(text);
            if (!fmMatch) {
                problems.push(`${skill} missing YAML frontmatter`);
                continue;
            }
            ...
            if (!nameMatch)
                problems.push(`${skill} missing frontmatter name`);
            else if (nameMatch[1].trim() !== skill)
                problems.push(`${skill} frontmatter name does not match folder`);
            if (!descriptionMatch || descriptionMatch[1].trim().length < 12)
                problems.push(`${skill} missing useful frontmatter description`);
        }
        if (problems.length === 0)
            add("PASS", "Active skill frontmatter is valid", "DOCTOR-SKILL-001");
        else
            add("FAIL", `Active skill frontmatter problems: ${problems.slice(0, 8).join("; ")}`, "DOCTOR-SKILL-001");
    }
    // TABLE-001
    {
                        ...
                        if (tableLines.length >= 2) {
                            ...
                            for (let j = 1; j < tableLines.length; j++) {
                                ...
                                if (cellCount !== headerCount)
                                    problems.push(`${file} line ${tableStart + j}: expected ${headerCount} columns, got ${cellCount}`);
                            }
                        }
                        tableStart = -1;
                        tableLines = [];
        ...
        if (problems.length === 0)
            add("PASS", "Markdown table column counts are consistent", "TABLE-001");
        else
            add("FAIL", `Markdown table column mismatch: ${problems.slice(0, 8).join("; ")}`, "TABLE-001");
    }
    // DOCTOR-003 context map
    const cr = contextMapConfig?.["config_refs"];
    ...
    const rulesObj = validationRules?.["rules"] ?? {};
    if (rulesObj["STRUCT-001"] && rulesObj["ENUM-001"] && rulesObj["DOCTOR-001"])
        add("PASS", "Validation rule catalog parses from JSON runtime config", "DOCTOR-003");
    else
        add("FAIL", "Validation rule catalog is missing required runtime rules", "DOCTOR-003");
    const research = orchestrationPolicy?.["research"] ?? {};
    const uiDelivery = orchestrationPolicy?.["ui_delivery"] ?? {};
    const externalization = orchestrationPolicy?.["externalization"] ?? {};
    const changeControl = orchestrationPolicy?.["change_control"] ?? {};
    if (orchestrationPolicy && (research["modes"] ?? []).length > 0 && (research["depths"] ?? []).length > 0 && (research["providers"] ?? []).length > 0 && (uiDelivery["values"] ?? []).length > 0 && (externalization["classifications"] ?? []).length > 0 && (changeControl["classifications"] ?? []).length > 0) {
    ...
    for (const [name, cfg] of Object.entries(schemaVersionConfigs)) {
        ...
        if (!cfg["schema_version"])
            schemaVersionProblems.push(`${name} is missing schema_version`);
        else if (cfg["schema_version"] !== supportedSchemaVersion)
            schemaVersionProblems.push(`${name} has unsupported schema_version ${cfg["schema_version"]} (expected ${supportedSchemaVersion})`);
    }
    if (schemaVersionProblems.length === 0)
    ...
    if (missingFromCatalog.length === 0 && deadCatalogEntries.length === 0)
        ...
        if (deadCatalogEntries.length > 0)
            add("FAIL", `Dead catalog entries (in validation-rules.json but never emitted): ${deadCatalogEntries.join(", ")}`, "DOCTOR-007");
    }
    // DOCTOR-008 suggestion
    const actionableSeverities = ["fail", "fail_release", "warn"];
    const rulesWithoutSuggestion = [];
    for (const [name, entry] of Object.entries(rulesObj)) {
        const e = entry;
        if (!actionableSeverities.includes(String(e["severity"] ?? "")))
            continue;
        if (String(e["suggestion"] ?? "").trim() === "")
            rulesWithoutSuggestion.push(name);
    }
    if (rulesWithoutSuggestion.length === 0)
        add("PASS", "Every fail/warn rule in the catalog carries a suggestion", "DOCTOR-008");
    else
        add("FAIL", `Rules missing a suggestion: ${sortOrdinal(rulesWithoutSuggestion).join(", ")}`, "DOCTOR-008");
    // DOCTOR-009 documentation
    const rulesWithBrokenDocs = [];
    for (const [name, entry] of Object.entries(rulesObj)) {
        const docPath = String(entry["documentation"] ?? "");
        if (!docPath.trim())
            continue;
        if (!existsSync(join(repo, docPath)))
            rulesWithBrokenDocs.push(`${name} -> ${docPath}`);
    }
    if (rulesWithBrokenDocs.length === 0)
        add("PASS", "Every rule documentation path resolves to a file", "DOCTOR-009");
    else
        add("FAIL", `Rules with unresolvable documentation: ${sortOrdinal(rulesWithBrokenDocs).join(", ")}`, "DOCTOR-009");
    // DOCTOR-013 onboarding questions
    if (policy && onboardingQuestions) {
        const declaredTriggers = policy["enums"]?.["strict_triggers"] ?? [];
        const questionKeys = Object.keys(onboardingQuestions["questions"] ?? {});
        ...
        if (triggersWithoutQuestion.length === 0 && questionsWithoutTrigger.length === 0)
            add("PASS", `onboarding-questions.json covers exactly the declared strict triggers (${declaredTriggers.length})`, "DOCTOR-013");
        else {
            if (triggersWithoutQuestion.length > 0)
                add("FAIL", `Strict triggers with no onboarding question: ${sortOrdinal(triggersWithoutQuestion).join(", ")}`, "DOCTOR-013");
            if (questionsWithoutTrigger.length > 0)
                add("FAIL", `onboarding-questions.json entries for triggers no longer declared: ${sortOrdinal(questionsWithoutTrigger).join(", ")}`, "DOCTOR-013");
        }
    }
    else {
    ...
    const experimentalBlocking = [];
    for (const [name, entry] of Object.entries(rulesObj)) {
        const e = entry;
        if (e["lifecycle"] === undefined)
            continue;
        if (String(e["lifecycle"]) !== "experimental")
            continue;
        if (blockingSeverities.includes(String(e["severity"] ?? "")))
            experimentalBlocking.push(name);
    }
    if (experimentalBlocking.length === 0)
    ...
    if (existsSync(settingsPath)) {
        ...
        try {
            ...
            add("PASS", ".claude/settings.json parses as JSON", "PERMISSION-000");
        }
        catch {
            add("FAIL", ".claude/settings.json is not valid JSON", "PERMISSION-000");
        }
        const permissions = settingsJson?.["permissions"] ?? {};
        const allowEntries = (permissions["allow"] ?? []).map(String);
        const askEntries = (permissions["ask"] ?? []).map(String);
        const denyEntries = (permissions["deny"] ?? []).map(String);
        const allowText = allowEntries.join("\n");
        if (/git push|git commit|git tag/.test(allowText))
            add("FAIL", ".claude/settings.json still allows git push/commit/tag", "PERMISSION-001");
        else
            add("PASS", ".claude/settings.json does not allow git push/commit/tag by default", "PERMISSION-001");
        const askText = askEntries.join("\n");
        if (/git push/.test(askText) && /git commit/.test(askText) && /git tag/.test(askText))
            add("PASS", ".claude/settings.json asks before git commit/push/tag", "PERMISSION-003");
        else
            add("FAIL", ".claude/settings.json should ask before git commit/push/tag", "PERMISSION-003");
        const denyText = denyEntries.join("\n");
        if (/\.env/.test(denyText) && /\.pem/.test(denyText) && /\.pfx/.test(denyText) && /id_rsa/.test(denyText) && /id_ed25519/.test(denyText))
            add("PASS", ".claude/settings.json denies common secret and private key paths", "PERMISSION-004");
        else
            add("FAIL", ".claude/settings.json should deny common secret and private key paths", "PERMISSION-004");
        if (settingsJson && settingsJson["disableBypassPermissionsMode"] === "disable")
        ...
        if (/\bWebSearch\b/.test(allowText))
            ...
            add("PASS", ".claude/settings.json asks before WebSearch", "PERMISSION-006");
        else
            add("WARN", ".claude/settings.json does not mention WebSearch", "PERMISSION-006");
        if (settings.includes("echo 'PMO-"))
            add("FAIL", "Fake PMO echo hooks still exist", "DOCTOR-HOOK");
        else
            add("PASS", "No fake PMO echo hooks found in settings", "DOCTOR-HOOK");
    }
    else {
    ...
    if (existsSync(gitignorePath)) {
        ...
        if (missingIgnores.length === 0 && staleIgnores.length === 0 && !/\*\*\/\*secret\*|\*\*\/\*credential\*/.test(gitignore))
            add("PASS", ".gitignore uses precise sensitive-file patterns", "PERMISSION-007");
        else
            add("FAIL", `.gitignore sensitive patterns need cleanup: missing=${missingIgnores.join(",")} stale=${staleIgnores.join(",")}`, "PERMISSION-007");
    }
    ...
    for (const skill of legacySkillNames) {
        if (claude.includes(skill) || agent.includes(skill))
            add("FAIL", `Router or core rules reference archived legacy skill: ${skill}`, "DOCTOR-002");
    }
        ...
        for (const file of getMarkdownFiles(skillPath)) {
            ...
            for (const pattern of legacyPatterns) {
                if (content.includes(pattern))
                    hits.push(`${skill}/${basename(file)}:${pattern}`);
            }
            ...
            if (unprohibited.length > 0)
                hits.push(`${skill}/${basename(file)}:log every`);
        }
        if (hits.length > 0)
            add("FAIL", `Active skill has legacy rule references: ${hits.slice(0, 5).join(", ")}`, "DOCTOR-004");
    }
    // Broken links (DOCTOR-000 default)
    const links = [];
    for (const file of getMarkdownFiles(repo)) {
        const relFile = file.substring(repo.length).replace(/^[/\\]/, "");
        if (/^tests[\\/]fixtures[\\/]invalid-/.test(relFile))
            continue;
        if (/(^|[\\/])(source|MOM|REQ|Transcript)[\\/]/.test(relFile))
            continue;
        const content = readFileSync(file, "utf8");
    ...
    if (links.length === 0)
        add("PASS", "No broken local markdown links found");
    else
        add("WARN", `Broken local links: ${links.slice(0, 8).join(", ")}`);
    return { rows, pass, warn, fail };
}
export function formatDoctorText(repo, result) {
    const lines = [];
