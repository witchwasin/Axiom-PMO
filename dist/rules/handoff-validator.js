...
import { getTableRowsAfterHeading, getTableLinesAfterHeading, splitReferenceValues } from "../markdown/table-parser.js";
...
function resolveLeadingReference(value, decisionIds, requirementIds, ctx) {
...
function testTableHeader(acc, catalog, text, heading, level, expected, artifact, handoffPolicy, state, isFullSpecDepth = false) {
    const policy = handoffPolicy["table_headers"];
    if (!policy || policy["enforce"] !== true)
        return true;
    ...
    if (actual.join("|") === expectedList.join("|"))
        return true;
...
function getDeclaredProjectCode(projectText, handoffPolicy) {
    const pattern = String(handoffPolicy["project_identity"]?.["authority_pattern"] ?? "");
    if (!pattern.trim())
        return null;
    const m = new RegExp(pattern, "m").exec(projectText);
    return m ? m[1].trim() : null;
}
export function getSectionBody(text, heading, level = 2) {
    ...
    for (let i = start + 1; i < lines.length; i++) {
        if (new RegExp(`^\\s*#{1,${level}}\\s+`).test(lines[i]))
            break;
        body.push(lines[i]);
    }
    return body.join("\n");
}
export function testHandoffReadiness(acc, catalog, ctx, mode, gate, workItems, deliveryIds, projectReqIds, decisionIds, projectText) {
    const handoffPolicy = ctx.handoffPolicy;
    const state = { ownerProblems: 0, headerOk: true };
    const ownerPolicy = handoffPolicy["owner_policy"] ?? {};
    const ownerLevel = getHandoffPolicySeverity(ownerPolicy["severity_by_mode"] ?? null, mode);
    const handoffDoc = handoffPolicy["handoff_document"] ?? {};
    ...
    const metaProblems = [];
    let handoffTarget = "";
    for (const field of handoffDoc["metadata_fields"] ?? []) {
        const key = String(field["key"]);
        let isRequired = field["required"] === true;
        if (field["required_modes"])
            isRequired = (field["required_modes"] ?? []).includes(mode);
        if (!isRequired)
            continue;
        if (!(key in meta)) {
        ...
        if (key === "Handoff Target")
            handoffTarget = value.toLowerCase();
        if (key === "Mode" && value !== mode)
            metaProblems.push(`${key} says '${value}' but the effective mode is '${mode}'`);
        if (key === "Horizon" && !testDateValue(value))
            metaProblems.push(`${key} '${value}' is not an ISO-8601 date (yyyy-MM-dd)`);
        if (key === "Build Spec Ref" && !existsSync(join(ctx.project, value)))
            metaProblems.push(`${key} points at '${value}', which does not exist`);
    }
    ...
    if (handoffTarget && !(handoffPolicy["handoff_targets"] ?? []).includes(handoffTarget)) {
        addResult(acc, catalog, "FAIL", `Handoff Target '${handoffTarget}' is not one of: ${(handoffPolicy["handoff_targets"] ?? []).join(", ")}`, { ruleId: "HANDOFF-001", blocking: true, artifact: "HANDOFF.md", field: "Handoff Target" });
    }
    const buildSpecRequired = (handoffPolicy["required_artifacts"]?.[mode] ?? []).includes("DESIGN/BUILD-SPEC.md");
    ...
    for (const section of handoffDoc["sections"] ?? []) {
        ...
        if (section["table"] === true && section["columns"]) {
            if (!testTableHeader(acc, catalog, handoffText, heading, 2, section["columns"], "HANDOFF.md", handoffPolicy, state))
                headerOk = false;
        }
        let applies = true;
        if (section["required_targets"])
            applies = (section["required_targets"] ?? []).includes(handoffTarget);
        else if (section["required"] !== true)
            applies = false;
        if (!applies)
            continue;
        ...
        if (rows.length === 0 && minRows === 0 && section["allow_explicit_none"] === true) {
            ...
            if (body && new RegExp(`^\\s*${deferredNoneToken.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "im").test(body))
                continue;
        }
    ...
    for (const item of workItems) {
        if (!buildNowRefs.includes(item["ID"] ?? ""))
            continue;
        if (testGenericOwner(item["Owner"] ?? "", ownerPolicy))
            unownedItems.push(item["ID"]);
    }
...
function testHandoffProjectIdentity(acc, catalog, project, projectText, metadata, handoffPolicy) {
    const policy = handoffPolicy["project_identity"];
    if (!policy || policy["enforce"] !== true)
        return;
    ...
    if (!declared || !declared.trim())
        return;
    const mismatches = [];
    for (const target of policy["cross_checked"] ?? []) {
        ...
        if (!actual || !actual.trim())
            continue;
        if (actual.trim() !== declared)
            mismatches.push({ artifact, field, value: actual.trim() });
    }
...
function testHandoffBuildSequence(acc, catalog, handoffText, rows, buildNowRefs, deliveryIds, ownerPolicy, ownerLevel, state) {
    if (rows.length === 0)
        return;
    const problems = [];
    const stepOf = {};
    ...
    for (const row of rows) {
        ...
        if (seenSteps[step])
            problems.push(`step ${step} is used more than once`);
        seenSteps[step] = true;
        for (const ref of splitReferenceValues(row["Work Item Ref"] ?? ""))
            stepOf[ref] = Number.parseInt(step, 10);
    }
    for (const row of rows) {
        const refs = splitReferenceValues(row["Work Item Ref"] ?? "");
        for (const ref of refs) {
            if (!deliveryIds.includes(ref))
                problems.push(`${ref} is sequenced but is not a DELIVERY.md work item`);
        }
        if (testGenericOwner(row["Owner"] ?? "", ownerPolicy)) {
    ...
    for (const ref of buildNowRefs) {
        if (stepOf[ref] === undefined)
            problems.push(`${ref} is in Build Now but has no place in the build sequence`);
    }
...
function testHandoffOpenActions(acc, catalog, rows, handoffPolicy, ownerPolicy, ownerLevel) {
    if (rows.length === 0) {
        addResult(acc, catalog, "PASS", "No open actions declared at handoff", { ruleId: "HANDOFF-009" });
        return;
    }
    const validPoints = handoffPolicy["blocking_points"] ?? [];
    let problems = 0;
    for (const row of rows) {
    ...
    if (problems === 0)
        addResult(acc, catalog, "PASS", "Every open action has an owner and a blocking point", { ruleId: "HANDOFF-009" });
}
function testHandoffDemoMilestone(acc, catalog, rows, handoffPolicy, ownerPolicy) {
    const lookup = {};
    for (const row of rows) {
        const key = (row["Field"] ?? "").trim();
        if (key.length > 0)
            lookup[key] = (row["Value"] ?? "").trim();
    }
    const missing = [];
    for (const field of handoffPolicy["handoff_document"]["demo_milestone_fields"] ?? []) {
        const key = String(field["key"]);
        ...
        if (field["owner_field"] === true && testGenericOwner(lookup[key], ownerPolicy))
            missing.push(`${key} (generic, not a named person)`);
    }
    if (missing.length > 0) {
...
function testHandoffEnvironmentMatrix(acc, catalog, rows, handoffPolicy, decisionIds, ctx) {
    if (rows.length === 0)
        return;
    const unresolved = handoffPolicy["environment_capabilities"]?.["unresolved_tokens"] ?? [];
    const problems = [];
    for (const row of rows) {
        ...
        for (const column of ["Serving Model", "Decision Ref"]) {
            ...
            for (const token of unresolved) {
                if (value.toLowerCase() === token.trim().toLowerCase())
                    isUnresolved = true;
            }
            if (isUnresolved) {
...
function testBuildSpec(acc, catalog, project, mode, handoffTarget, handoffPolicy, projectReqIds, decisionIds, projectText, ctx, state) {
    const spec = handoffPolicy["build_spec"];
    ...
    const minWords = Number(spec["not_required_policy"]["min_rationale_words"]);
    const isFullSpecDepth = /^\s*>?\s*Spec depth:\s*full\s*$/im.test(projectText);
    const sectionProblems = [];
    for (const section of spec["sections"] ?? []) {
        const heading = String(section["heading"]);
        let applies = false;
        if (section["required_modes"] && (section["required_modes"] ?? []).includes(mode))
            applies = true;
        if (section["required_targets"] && (section["required_targets"] ?? []).includes(handoffTarget))
            applies = true;
        if (applies && section["required_declarations"]) {
            for (const declaration of section["required_declarations"] ?? []) {
                if (!new RegExp(`^\\s*>?\\s*${String(declaration).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}:`, "m").test(projectText)) {
                    applies = false;
                    break;
                }
            }
        ...
        const statusValues = spec["status_values"] ?? [];
        ...
        if (status === "not_required") {
            ...
            if (isFullSpecDepth && Array.isArray(section["allow_not_required_modes"])) {
                isAllowed = section["allow_not_required_modes"].includes(mode);
            }
            ...
            if (rationale.length === 0 || testPlaceholderValue(rationale))
                sectionProblems.push(`'${heading}' is marked not_required without a ${rationaleMarker}`);
            else if (rationale.split(/\s+/).length < minWords)
                sectionProblems.push(`'${heading}' has a ${rationaleMarker} shorter than ${minWords} words`);
            continue;
        }
        ...
        if (section["table"] === true) {
            if (!isFullSpecDepth && (heading === "API or Command Contract" || heading === "State Machine and Transition Guards")) {
                ...
                if (section["columns"]) {
                    if (!testTableHeader(acc, catalog, text, heading, 3, section["columns"], "DESIGN/BUILD-SPEC.md", handoffPolicy, state, isFullSpecDepth)) {
                        state.headerOk = false;
                    }
                }
            }
...
function testBuildSpecAcceptanceCases(acc, catalog, text, handoffPolicy, projectReqIds) {
    const rows = getTableRowsAfterHeading(text, getHeadingPattern("Acceptance Cases", 3));
    if (rows.length === 0)
        return;
    const classes = handoffPolicy["acceptance_cases"]?.["execution_classes"] ?? [];
    let missingClass = 0;
    ...
    if (missingClass === 0)
        addResult(acc, catalog, "PASS", "Every acceptance case is classified automated or manual", { ruleId: "HANDOFF-006" });
    if (missingFixture === 0)
        addResult(acc, catalog, "PASS", "Every acceptance case declares seed and reset strategy", { ruleId: "HANDOFF-007" });
}
function testBuildSpecDataInventory(acc, catalog, text, handoffPolicy, decisionIds, ctx) {
    const rows = getTableRowsAfterHeading(text, getHeadingPattern("Security, Privacy and Data Inventory", 3));
    if (rows.length === 0)
        return;
    const sensitiveData = handoffPolicy["sensitive_data"] ?? {};
    const yesValues = sensitiveData["declared_yes_values"] ?? [];
    const noValues = sensitiveData["declared_no_values"] ?? [];
    ...
    if (problems === 0)
        addResult(acc, catalog, "PASS", "Every data element declared sensitive carries a classification and retention decision", { ruleId: "HANDOFF-011" });
}
function testBuildSpecCapabilities(acc, catalog, text, handoffPolicy, decisionIds, ctx) {
    ...
    if (problems === 0)
        addResult(acc, catalog, "PASS", "Every declared runtime capability has a resolved serving model and environment decision", { ruleId: "HANDOFF-012" });
}
function testHandoffSemanticReview(acc, catalog, project, mode, handoffPolicy, projectText, decisionIds, ctx) {
    const reviewPolicy = handoffPolicy["semantic_review"] ?? {};
    const reviewPath = join(project, String(reviewPolicy["artifact"]));
    const missingLevel = getHandoffPolicySeverity(reviewPolicy["severity_when_missing"] ?? null, mode, "warn");
    const isRequired = (reviewPolicy["required_modes"] ?? []).includes(mode);
    ...
    const problems = [];
    if (String(review["schema_version"] ?? "") !== String(reviewPolicy["schema_version"]))
        problems.push(`schema_version is '${review["schema_version"]}', expected '${reviewPolicy["schema_version"]}'`);
    if (!(reviewPolicy["reviewer_kinds"] ?? []).includes(String(review["reviewer_kind"] ?? "")))
        problems.push(`reviewer_kind '${review["reviewer_kind"]}' is not one of: ${(reviewPolicy["reviewer_kinds"] ?? []).join(", ")}`);
    const policyLensIds = (reviewPolicy["lenses"] ?? []).map((l) => String(l["id"]));
    const reviewedLensIds = (review["lenses"] ?? []).map((l) => String(l["lens"]));
    ...
    if (missingLenses.length > 0)
        problems.push(`lenses not reviewed: ${missingLenses.join(", ")}`);
    if (unknownLenses.length > 0)
        problems.push(`unknown review lenses: ${sortOrdinalUnique(unknownLenses).join(", ")}`);
    const validSeverities = reviewPolicy["finding_severities"] ?? [];
    const validStatuses = reviewPolicy["finding_statuses"] ?? [];
    const validPoints = handoffPolicy["blocking_points"] ?? [];
    const closure = reviewPolicy["closure_policy"] ?? null;
    if (!closure)
        throw new Error("handoff-policy.json semantic_review is missing closure_policy; refusing to guess who may close a finding.");
    const openStatuses = closure["open_statuses"] ?? [];
    const aiClosable = closure["ai_closable_statuses"] ?? [];
    const humanOnlyLenses = closure["human_only_close_lenses"] ?? [];
    const needsDecisionRef = closure["statuses_requiring_decision_ref"] ?? [];
    const reviewerIsAi = String(review["reviewer_kind"] ?? "") === "ai";
    const ownerPolicy = handoffPolicy["owner_policy"] ?? {};
    for (const finding of review["findings"] ?? []) {
        const id = String(finding["finding_id"] ?? "").trim();
        if (id.length === 0) {
            problems.push("a finding has no finding_id");
            continue;
        }
        const lens = String(finding["lens"] ?? "").trim();
        const status = String(finding["status"] ?? "").trim();
        if (!validSeverities.includes(String(finding["severity"] ?? "")))
            problems.push(`${id} has severity '${finding["severity"]}'`);
        if (!validStatuses.includes(status))
            problems.push(`${id} has status '${status}'`);
        if (!validPoints.includes(String(finding["blocking_point"] ?? "")))
            problems.push(`${id} has blocking_point '${finding["blocking_point"]}'`);
        if (lens.length === 0 || !policyLensIds.includes(lens))
            problems.push(`${id} is filed under unknown lens '${lens}'`);
        if (testGenericOwner(String(finding["owner"] ?? ""), ownerPolicy))
            problems.push(`${id} has no named owner`);
        if ((finding["evidence_refs"] ?? []).length === 0)
            problems.push(`${id} cites no evidence`);
        if (!String(finding["suggestion"] ?? "").trim())
            problems.push(`${id} offers no suggestion`);
        if (openStatuses.includes(status))
            continue;
        const decisionRef = String(finding["decision_ref"] ?? "").trim();
        if (needsDecisionRef.includes(status)) {
            if (decisionRef.length === 0)
                problems.push(`${id} is '${status}' with no decision_ref -- a finding is only closed by a decision somebody recorded`);
            else if (!resolveLeadingReference(decisionRef, decisionIds, null, ctx))
                problems.push(`${id} is '${status}' but decision_ref '${decisionRef}' does not resolve`);
        }
        if (reviewerIsAi) {
            if (humanOnlyLenses.includes(lens))
                problems.push(`${id} is '${status}' under lens '${lens}', which only a human may close -- an AI reviewer must leave it open and name the person who decides`);
            else if (!aiClosable.includes(status))
                problems.push(`${id} is '${status}', which an AI reviewer may not set (allowed: ${aiClosable.join(", ")})`);
        }
        if (humanOnlyLenses.includes(lens) && closure["human_only_requires_decision_log_entry"] === true) {
            const dm = /(DEC-\d{3})/.exec(decisionRef);
            if (!dm)
                problems.push(`${id} is '${status}' under human-only lens '${lens}' and cites no decision-log entry -- closure there must reference a DEC-### somebody signed`);
            else {
                const decisionId = dm[1];
                const decider = getDecisionDecider(project, decisionId);
                if (decider === null)
                    problems.push(`${id} cites ${decisionId}, which is not a row in decision-log.md`);
                else if (testGenericOwner(decider, ownerPolicy))
                    problems.push(`${id} closes a human-only lens against ${decisionId}, whose decider is '${decider}' rather than a named person`);
            }
        }
    }
    const staleLevel = getHandoffPolicySeverity(reviewPolicy["freshness"]?.["stale_severity"] ?? null, mode, "warn");
    let isStale = false;
    const currentSourceDigest = getSourceSnapshotDigest(projectText);
    const recordedSourceDigest = String((review["source_snapshot"] ?? {})["digest"] ?? "").trim().toLowerCase();
    if (!recordedSourceDigest)
        problems.push("source_snapshot.digest is not recorded, so freshness cannot be checked");
    ...
    const recordedInputDigest = String((review["review_inputs"] ?? {})["digest"] ?? "").trim().toLowerCase();
    if (!recordedInputDigest)
        problems.push("review_inputs.digest is not recorded, so changes to the reviewed artifacts cannot be detected");
    ...
    if (problems.length > 0) {
        ...
        const openCritical = (review["findings"] ?? []).filter((f) => String(f["status"]) === "open" && String(f["severity"]) === "critical");
...
export function testEarlyTestDesign(acc, catalog, project, mode, gate, projectReqIds, projectText, handoffPolicy) {
    if (mode === "Lite" || !["Design", "Handoff"].includes(gate))
        return;
    if (!/^\s*>?\s*(Research mode|UI delivery):/m.test(projectText))
        return;
    ...
    const ownerPolicy = handoffPolicy["owner_policy"] ?? {};
    const bad = rows.filter((r) => testPlaceholderValue(String(r["Test Area"] ?? "")) ||
    ...
    if (bad.length > 0)
        addResult(acc, catalog, "FAIL", "BUILD-SPEC Test Strategy contains incomplete cases", { ruleId: "TEST-DESIGN-001", artifact: "DESIGN/BUILD-SPEC.md", field: "Test Strategy" });
    else
        addResult(acc, catalog, "PASS", "BUILD-SPEC carries mode-aware test strategy", { ruleId: "TEST-DESIGN-001" });
    ...
    if (missing.length > 0)
        addResult(acc, catalog, "FAIL", `Test Strategy does not cover scoped requirements: ${missing.join(", ")}`, { ruleId: "TEST-DESIGN-002", artifact: "DESIGN/BUILD-SPEC.md", field: "Test Strategy" });
    else
        addResult(acc, catalog, "PASS", "Test Strategy covers applicable scoped requirements", { ruleId: "TEST-DESIGN-002" });
}
