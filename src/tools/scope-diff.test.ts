...
function runScopeDiff(project: string, repoRoot: string, base: string, head: string): { result: ScopeDiffResult; diagnostics: Diagnostic[] } {
...
function scopeDiffRows(diagnostics: Diagnostic[], ruleId: string, level: "" | "PASS" | "WARN" | "FAIL" = ""): Diagnostic[] {
...
test("scope-diff: verdicts -- in scope passes, one file outside fails", () => {
  ...
  try {
    ...
    assert.equal(
      scopeDiffRows(r.diagnostics, "SCOPE-DIFF-001", "FAIL").filter((d) => d.artifact === "src/auth/bar.ts").length, 1,
      "one-outside: SCOPE-DIFF-001 FAIL row names the file as artifact",
    );
  } finally {
...
test("scope-diff: excluded files and include/exclude precedence", () => {
  ...
  try {
    ...
    assert.ok(
      r.result.changed_excluded.includes("src/payments/generated/client.ts") && !r.result.changed_out_of_scope.includes("src/payments/generated/client.ts"),
      "excluded: reported under changed_excluded, not out_of_scope",
    );
...
test("scope-diff: missing and invalid scope declarations", () => {
  ...
  try {
    ...
    assert.equal(
      r.diagnostics.some((d) => d.level === "FAIL") ? 1 : 0, 1,
      "missing scope: overall exit code is 1 (not silently passing)",
    );
...
test("scope-diff: added / modified / deleted files and renames", () => {
  ...
  try {
    ...
    assert.ok(
      renameRow[0]!.old_path === "src/payments/old name.ts" && renameRow[0]!.old_verdict === "in_scope" && renameRow[0]!.new_verdict === "in_scope",
      "rename in-scope-to-in-scope: structured entry has both verdicts",
    );
    ...
    assert.match(
      scopeDiffRows(r.diagnostics, "SCOPE-DIFF-001", "FAIL")[0]!.message, /renamed from/,
      "rename: violation message mentions the rename",
    );
    ...
    assert.ok(
      renameRow2[0]!.old_path === "src/payments/old name.ts" && renameRow2[0]!.old_verdict === "in_scope" && renameRow2[0]!.new_verdict === "out_of_scope",
      "rename in-scope-to-out-of-scope: old side recorded in_scope, new side out_of_scope",
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test("scope-diff: special-character paths and git error handling", () => {
  ...
  try {
    ...
    let r = runScopeDiff(dir, dir, base, head);
    assert.ok(
      r.result.changed_in_scope.includes("src/with space (and parens) [brackets].ts"),
      `special characters: path parsed and matched correctly (got: ${r.result.changed_in_scope.join(", ")})`,
    );
...
test("scope-diff: repo-wide exemptions", () => {
  ...
  try {
    ...
    assert.equal(
      r.result.exempt.filter((e) => e.path === "package-lock.json" && e.reason).length, 1,
      "repo-wide exempt: file listed with a reason",
    );
...
test("scope-diff: repo-wide exemption policy validation", () => {
  ...
  try {
    ...
    const writePolicy = (json: string) => writeFileSync(policyPath, json, "utf8");
...
test("scope-diff: opt-in -- omitted refs change nothing", () => {
  ...
  try {
    ...
    assert.equal(
      result.diagnostics.filter((d) => d.rule_id.startsWith("SCOPE-DIFF-")).length, 0,
      "opt-in: no SCOPE-DIFF rows appear when not requested",
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
