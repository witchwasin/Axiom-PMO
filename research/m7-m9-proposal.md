...
> **Status: proposal, revision 3 (final draft). Not authorized, not started.**
>
> Revisions 1 and 2 were each reviewed by Sol. Round 2's verdict was ACCEPT
> WITH MINOR REVISIONS, with two blocking corrections; both are resolved below,
> one of them differently from the way it was proposed and with the evidence for
> doing so. §0 records the disposition of every finding from both rounds.
>
> Sol's reviews are **candidate evidence, not authorization.** They recommend
> approving M7; only the Human Owner can approve it, and nothing here has been
> authorized. The decisions that remain open are in §7.
>
> Each milestone section is written in the roadmap's own planning shape so it
> can be pasted into `ROADMAP.md` with minimal editing.
>
> Author: AI (Claude Opus 5), 2026-08-03. Sources: the Human Owner's two written
> proposals of 2026-08-03; Sol's reviews of revisions 1 and 2.
...
| Sol's point | Disposition | Reason |
|---|---|---|
...
| Path rules must not demand execution artifacts before execution | **Accepted; fixed differently** | Right problem, wrong fix. `validate-project.ps1` contains **zero** references to `EXEC-*` or execution artifacts — no gate *can* demand them. Sol's example also keyed off an `"Execution"` gate absent from `policy.json enums.gates`. The offending rule was deleted |
...
| **B2** | **Blocking.** The finding lifecycle says the executor may set `disputed` *and* may never move a finding out of `open` | **Accepted in full — a straight contradiction in revision 2's own text** | Fixed with Sol's wording, plus the missing property: `disputed` is not a closure and remains blocking |
...
must not create files there. Sol's finding is accepted in full.

Sol proposed two remedies — `.axiom/onboarding/declaration.json`, or
...
## 7. Decisions needed from the Human Owner

Nothing below can be defaulted by an AI. Sol's reviews authorize none of it.

1. **Authorize M7 implementation** as revised. Proposed record: `DEC-010`.
2. **Confirm the execution-path contract**: `execution_path` is a governed
   ...
   `development_handoff`.
3. **Confirm the declaration mechanism**: onboarding answers are recorded in the
...
4. **Authorize M8.0 research only** — threat model plus GO/NO-GO, with the
   `ci-observed` provenance question as its primary subject. NO-GO is an
   acceptable outcome. Proposed record: `DEC-011`.
5. **Confirm that a `human-attested` review satisfies Strict**, so Strict never
   requires purchasing a second model — with the stated caveat that the
   framework records attested accountability, not proven identity separation.
6. **Confirm M9 is local and opt-in**, with no network transmission by default
   or implicitly, events git-ignored by default, and any future external
   aggregation a separate milestone and decision.
7. **Add the `Permanent Non-Goals` section** in §5.

Items 1–7 are decisions. Everything else here is design detail that can be
