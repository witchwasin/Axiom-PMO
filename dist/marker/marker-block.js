...
export function getAxiomBlockDigest(content) {
    const normalized = content.replace(/\r\n/g, "\n").trim();
    return createHash("sha256").update(normalized, "utf8").digest("hex").toLowerCase();
}
export function findAxiomBlock(text) {
    ...
    if (begins.length > 1)
        return { status: "malformed", reason: `the file contains ${begins.length} Axiom-PMO BEGIN markers; exactly one is expected` };
    if (ends.length > 1)
        return { status: "malformed", reason: `the file contains ${ends.length} Axiom-PMO END markers; exactly one is expected` };
    if (begins.length === 1 && ends.length === 0)
        return { status: "malformed", reason: "an Axiom-PMO BEGIN marker has no matching END marker" };
    if (begins.length === 0 && ends.length === 1)
        return { status: "malformed", reason: "an Axiom-PMO END marker has no matching BEGIN marker" };
    if (ends[0].index < begins[0].index)
        return { status: "malformed", reason: "the Axiom-PMO END marker appears before its BEGIN marker" };
...
export function testAxiomBlockOwnership(block) {
    if (block.status !== "present")
        return "absent";
    const actual = getAxiomBlockDigest(block.content ?? "");
    if (KNOWN_BODY_DIGESTS.includes(actual))
        return "owned";
    if (!block.digest)
        return "unknown";
    if (block.digest === actual)
        return "foreign";
    return "edited";
}
export function getAxiomOwnershipReason(ownership, verb = "modify") {
    switch (ownership) {
        case "edited":
...
export function newAxiomBlockText(body, newline = "\n") {
...
export function setAxiomBlock(text, body, newline = "\n", force = false) {
...
export function removeAxiomBlock(text, _newline = "\n", force = false) {
    const block = findAxiomBlock(text);
    if (block.status === "absent")
        return { action: "absent", text };
    if (block.status === "malformed")
        return { action: "blocked", reason: block.reason, text };
    const ownership = testAxiomBlockOwnership(block);
    if (ownership !== "owned" && !force) {
