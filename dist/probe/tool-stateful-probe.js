    ...
    for (const c of configs) {
        ...
        try {
            ...
            const psArgs = ["-ProjectCode", code, "-Mode", c.args.mode, "-ExecutionPath", c.args.executionPath, "-ResearchMode", c.args.researchMode,
                ...
                "-OutputRoot", psRoot];
            if (c.args.includeHandoff)
                psArgs.push("-IncludeHandoff", "-Target", c.args.target, "-HorizonDays", String(c.args.horizonDays));
...
// ---------------------------------------------------------------------------
{
    ...
    try {
        ...
        check("update-source-snapshot write: output", norm(ref.stdout, psDir) === norm(cand.output, tsDir), getGoldenDiffReport(norm(ref.stdout, psDir), norm(cand.output, tsDir)).join(" | "));
        ...
        check("update-source-snapshot write: PROJECT.md identical (modulo timestamp)", psProj === tsProj, getGoldenDiffReport(psProj, tsProj).join(" | "));
...
// ---------------------------------------------------------------------------
{
    ...
    try {
        ...
        check("aggregate-diagnostics: registry identical (modulo timestamps)", refCanon === candCanon && refCanon !== null, refCanon === candCanon ? "" : `ref=${refCanon} cand=${candCanon}`);
        ...
        check("aggregate-diagnostics: event governed fields identical", JSON.stringify(keysA) === JSON.stringify(keysB), `ps=${JSON.stringify(keysA)} ts=${JSON.stringify(keysB)}`);
    }
    finally {
...
// ---------------------------------------------------------------------------
{
    ...
    try {
        ...
        check("clean-room install: output", norm(ref.stdout, psDir) === norm(cand.output, tsDir), getGoldenDiffReport(norm(ref.stdout, psDir), norm(cand.output, tsDir)).join(" | "));
