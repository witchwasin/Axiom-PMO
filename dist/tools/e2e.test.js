...
import { mkdtempSync, mkdirSync, rmSync, writeFileSync, readFileSync, existsSync, appendFileSync, } from "node:fs";
...
function rep(text, pattern, replacement, flags = "g") {
...
function fillProjectContent(project, mode, projectCode, today) {
...
function fillHandoffContent(project, mode, projectCode, today) {
...
// gate.
function assertGatesPass(project, mode, gates, label) {
    for (const gate of gates) {
        ...
        assert.equal(problems.length, 0, `${label} failed validation at Gate=${gate}: ${problems.slice(0, 6).map((p) => `${p.rule_id}: ${p.message}`).join(" | ")}`);
    }
}
function generate(projectCode, mode, workRoot, includeHandoff) {
    const gen = newProject(REPO_ROOT, projectCode, mode, "development_handoff", "off", "standard", "none", "not_applicable", "none", "normal feature", "PM", workRoot, includeHandoff, "demo", 21);
...
test("Handoff E2E: generate with scaffolding -> fill -> all gates -> assess -> staleness", () => {
    ...
    try {
        ...
        assert.ok(scaffoldDiags.some((d) => d.level === "FAIL"), "Handoff E2E: a freshly generated, unfilled scaffold passed the Handoff gate");
        ...
        const parsed = JSON.parse(assessment.output);
