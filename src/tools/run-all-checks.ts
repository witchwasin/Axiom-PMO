...
export function runAllChecks(repoRoot: string, testChildScript: string): RunAllChecksResult {
  ...
  function invokeCheck(name: string, exe: string, args: string[]): number {
    out.push(`[CHECK] ${name}`);
    ...
    if (exitCode !== 0) {
      ...
      if (process.env.GITHUB_ACTIONS === "true") out.push(`::error title=Axiom-PMO check failed::${name} exited ${exitCode}`);
      return exitCode;
    }
  ...
  function recordResult(name: string, text: string, exitCode: number): number {
    out.push(`[CHECK] ${name}`);
  ...
  const exampleChecks: Array<[string, string, Mode, Gate]> = [
