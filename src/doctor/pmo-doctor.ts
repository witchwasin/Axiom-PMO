...
export function runPmoDoctor(repo: string, skillRootOverride = "", templateRootOverride = ""): DoctorResult {
  ...
  if (existsSync(srcDir) && statSync(srcDir).isDirectory()) emitterFiles.push(...walkTs(srcDir));
  ...
  for (const file of emitterFiles) {
    ...
    for (const line of readFileSync(file, "utf8").split(/\r?\n/)) {
      ...
      if (!/addResult|\badd\(|\.add\(|testReviewRow|ruleId:|rule_id:|ruleId\s*=/.test(line)) continue;
  ...
  if (missingFromCatalog.length === 0 && deadCatalogEntries.length === 0) add("PASS", "Validation rule catalog matches the rule ids emitted by the Node validators", "DOCTOR-007");
  else {
  ...
  // DOCTOR-015 template ↔ policy drift guard
  {
    ...
    if (existsSync(buildSpecPath) && handoffPolicy?.["build_spec"]) {
      ...
      const declaredSections = ((handoffPolicy["build_spec"] as Record<string, unknown>)["sections"] as Array<Record<string, unknown>>) ?? [];
      ...
      if (declaredHeadings.join("|") !== headingMatches.join("|")) {
        ...
        if (missingInTemplate.length > 0) problems.push(`Missing sections in BUILD-SPEC.md template: ${missingInTemplate.join(", ")}`);
        if (unexpectedInTemplate.length > 0) problems.push(`Unexpected sections in BUILD-SPEC.md template: ${unexpectedInTemplate.join(", ")}`);
      ...
      for (const section of declaredSections) {
        if (section["table"] !== true || !Array.isArray(section["columns"])) continue;
        const heading = String(section["heading"] ?? "");
        const expectedCols = (section["columns"] as string[]).map((c) => c.trim());
  ...
  if (existsSync(settingsPath)) {
    ...
    if (/node cli\/axiom\.mjs/.test(allowText)) add("PASS", ".claude/settings.json allows the axiom CLI", "PERMISSION-002");
    else add("FAIL", ".claude/settings.json does not allow node cli/axiom.mjs", "PERMISSION-002");
