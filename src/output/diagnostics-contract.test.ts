...
interface DiagnosticsSchema {
  diagnostic: { required: string[] };
  envelope: { required: string[] };
  diagnostics_schema_version: string;
  compatibility: { stable_fields: string[] };
}
...
const schema = JSON.parse(readFileSync(join(REPO_ROOT, "pmo-config/diagnostics-schema.json"), "utf8")) as DiagnosticsSchema;
const catalog = JSON.parse(
  readFileSync(join(REPO_ROOT, "pmo-config/validation-rules.json"), "utf8").replace(/^﻿/, ""),
) as RuleCatalog;
...
const samples: Array<{ name: string; fixture: string; mode: Mode; gate: Gate }> = [
...
test("every diagnostic row matches the declared contract", () => {
  ...
  const suggestionProblems: string[] = [];
  const docProblems: string[] = [];
  const sensitiveProblems: string[] = [];
  ...
  for (const row of allResults) {
    ...
    if (missing.length > 0) shapeProblems.push(`${row.rule_id}: missing ${missing.join(", ")}`);
  ...
  for (const row of allResults) {
    ...
    for (const field of v1Fields) {
      if (!keys.includes(field)) v1Problems.push(`${row.rule_id} lost v1.0 field '${field}'`);
    }
  }
