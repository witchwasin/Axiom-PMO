...
function tsSetup(dir: string, action: "install" | "remove"): void {
  ...
  if (!state.supported) throw new Error(`unsupported encoding: ${state.encoding}`);
  const result = action === "install"
    ? setAxiomBlock(state.text, body, state.newline)
    : removeAxiomBlock(state.text, state.newline);
  if (result.action === "blocked") throw new Error(`blocked: ${result.reason}`);
  if (state.exists) newAxiomBackup(file);
...
function treeSnapshot(dir: string): Record<string, string> {
  ...
  const walk = (d: string) => {
    for (const entry of readdirSync(d)) {
      ...
      else out[full.substring(dir.length + 1)] = readFileSync(full).toString("base64");
    }
  };
  walk(dir);
  return out;
