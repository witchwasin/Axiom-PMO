...
import { getTableRowsAfterHeading, splitReferenceValues } from "../markdown/table-parser.js";
...
export function exportExecutionContract(repoRoot, projectPath, workItemId, gitRepoRoot, outputPath, grant, force) {
    const project = resolve(projectPath);
    const gitRoot = resolve(gitRepoRoot ?? project);
    const deliveryPath = join(project, "DELIVERY.md");
    if (!existsSync(deliveryPath))
        return failExport(`No DELIVERY.md in ${project}. An execution contract is generated from an approved work item; there is no work item board to read.`);
    const deliveryText = readFileSync(deliveryPath, "utf8");
    const workItems = getTableRowsAfterHeading(deliveryText, "^##\\s+Work Items");
    if (workItems.length === 0)
        return failExport("DELIVERY.md has no Work Items table.");
    const item = workItems.find((row) => row["ID"] === workItemId);
    if (!item)
        return failExport(`Work item '${workItemId}' is not in DELIVERY.md's Work Items table.`);
    ...
    if (existsSync(projectMdPath)) {
        ...
        if (m && m[1].trim())
            projectCode = m[1].trim();
    }
    const scope = readScopeDeclaration(project);
    if (!scope.present)
        return failExport(`No SCOPE.json in ${project}. An execution contract's allowed_paths are derived from the project's approved implementation scope -- declare it first (see templates/SCOPE.json). A contract must never grant broader path access than the approved scope.`);
    if (!scope.valid)
        return failExport(`SCOPE.json is invalid: ${scope.error}`);
    ...
    if (existsSync(adversarialReviewPolicyPath)) {
        ...
        const pinned = String(policy["externally_observed_binding"]?.["pinned_workflow_path"] ?? "");
        if (pinned.trim())
            reviewWorkflowPaths.push(pinned);
    }
    const baseSha = spawnSync("git", ["-C", gitRoot, "rev-parse", "--verify", "--quiet", "HEAD^{commit}"], { encoding: "utf8" }).stdout?.trim() ?? "";
    if (!baseSha)
        return failExport(`Could not resolve HEAD in ${gitRoot}. The contract pins an exact base commit (never a branch name, which moves); a repository with no resolvable HEAD cannot be exported from.`);
    const grantableActions = ["create_branch", "commit", "push", "merge", "deploy"];
    const gitAuthority = { create_branch: true, commit: false, push: false, merge: false, deploy: false };
    for (const piece of grant.split(",")) {
        const action = piece.trim();
        if (!action)
            continue;
        if (!grantableActions.includes(action))
            return failExport(`Unknown git action in -Grant: '${action}'. Grantable actions are: ${grantableActions.join(", ")}.`);
    ...
    if (existsSync(contractPath) && !force)
        return failExport(`${contractPath} already exists. Re-exporting would change the digest an existing result may already reference; pass -Force to overwrite deliberately.`);
