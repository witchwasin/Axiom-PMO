...
try {
    ...
    check("junction was created (path resolves through it)", statSync(junction).isDirectory(), "junction path did not resolve");
    ...
    check("lstat reports the junction as a reparse point", lstatSync(junction).isSymbolicLink(), "isSymbolicLink() was false for a junction -- the check as written cannot catch one");
    // ---- TS port (in-process) --------------------------------------------
    ...
    if (junctionCreated) {
        try {
            rmSync(join(sandbox, "proj"), { recursive: true, force: true });
        }
        catch { /* best effort */ }
    }
    try {
...
// findAncestorReparsePoint for the fix, which subsumes Case 1 above).
{
    ...
    try {
        ...
        if (ancestorJunctionCreated) {
            try {
                rmSync(join(ancestorSandbox, "linked-parent"), { recursive: true, force: true });
            }
            catch { /* best effort */ }
        }
        try {
            rmSync(ancestorSandbox, { recursive: true, force: true });
        }
        catch { /* best effort */ }
        try {
            rmSync(ancestorRealParent, { recursive: true, force: true });
        }
        catch { /* best effort */ }
    }
}
...
// the caller never named.
{
    ...
    try {
        ...
        if (emptyJunctionCreated) {
            try {
                rmSync(join(emptySandbox, "proj"), { recursive: true, force: true });
            }
            catch { /* best effort */ }
        }
        try {
        ...
        try {
            rmSync(emptyRealDir, { recursive: true, force: true });
        }
        catch { /* best effort */ }
    }
}
