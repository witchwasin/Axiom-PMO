...
test("config mutation proves JSON runtime config is the source of truth", () => {
    ...
    try {
        ...
        const validate = (project, mode, gate) => runPortedChain(tempRepo, join(tempRepo, project), mode, gate).diagnostics;
        // 1. policy enum mutation → ENUM-001
        const policyPath = join(tempRepo, "pmo-config/policy.json");
        let policy = readJson(policyPath);
        policy["enums"]["statuses"] = policy["enums"]["statuses"].filter((s) => s !== "Done");
        ...
        manifest["active_skills"] = manifest["active_skills"].filter((s) => s["id"] !== "pmo-intake");
        ...
        const matrix = artifactPolicy["artifact_matrix"]["Standard"];
        matrix["Release"] = [...matrix["Release"], "RTM.json"];
        ...
        orchestration["research"]["modes"] = orchestration["research"]["modes"].filter((m) => m !== "off");
        ...
        if (projectText.charCodeAt(0) === 0xfeff)
            projectText = projectText.slice(1); // strip BOM like PS Get-Content -Raw
        ...
        orch["externalization"]["internal_default_human_review"] = true;
        ...
        orch["research"]["research_statuses"] = orch["research"]["research_statuses"].filter((s) => s !== "stopped");
        writeJson(orchestrationPath, orch);
        ...
        handoff["owner_policy"]["generic_tokens"].push("R. Silva");
        ...
        handoff["semantic_review"]["lenses"] = [
            ...handoff["semantic_review"]["lenses"],
        ...
        rules["rules"]["STRUCT-001"]["lifecycle"] = "experimental";
        writeJson(rulesPath, rules);
        doctor = runPmoDoctor(tempRepo);
        ...
        handoff = readJson(handoffPath);
        const specSections = handoff["build_spec"]["sections"];
