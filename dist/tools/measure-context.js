...
export function measureContext(repoRoot, files) {
    const rows = [];
...
export function formatContextTable(rows) {
    const headers = ["File", "Lines", "Words", "EstimatedContextSize"];
    const cells = rows.map((r) => [r.file, String(r.lines), String(r.words), r.estimatedContextSize.toFixed(3)]);
    if (cells.length === 0) {
