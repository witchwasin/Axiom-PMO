...
export function getProjectExecutionPath(projectRoot) {
...
export function testExecutionPath(acc, catalog, project, policyEnums, workItems) {
    const declaredRaw = getProjectExecutionPath(project);
    const validPaths = policyEnums["execution_paths"] ?? [];
    ...
    if (effectivePath !== "development_handoff")
        return;
    const executionRoot = join(project, ".execution");
    if (!existsSync(executionRoot))
        return;
    const doneItemIds = new Set(workItems.filter((w) => w["Status"] === "Done").map((w) => w["ID"]));
