...
export function testApproval(
  ...
  if (status !== "approved") invalid.push("approval_status");
  if (testPlaceholderValue(approver ?? "")) invalid.push("approver");
  if (testPlaceholderValue(role ?? "")) invalid.push("role");
  if (testPlaceholderValue(date ?? "") || !testDateValue(date ?? "")) invalid.push("date");
  if (testPlaceholderValue(evidence ?? "")) invalid.push("evidence");
  ...
  if (!testPlaceholderValue(approver ?? "")) {
    const ownerPolicy = (ctx.handoffPolicy["owner_policy"] as Record<string, unknown>) ?? {};
    if (testGenericOwner(approver!, ownerPolicy)) {
      const severityMap = (ownerPolicy["severity_by_mode"] as Record<string, unknown>) ?? null;
  ...
  const allowedRoles = (ctx.policy["approval_roles"] as Record<string, unknown> | undefined)?.[gateName] as string[] | undefined;
...
// Re-export for source-validator's shared use.
