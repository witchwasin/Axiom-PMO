...
test("digests must not depend on line endings", () => {
    ...
    try {
        ...
        const lfDigest = getReviewInputDigest(sample, cfg.handoffPolicy);
        ...
        const walk = (dir) => {
            ...
            for (const entry of readdirSync(dir)) {
                ...
                if (st.isDirectory())
                    out.push(...walk(full));
                else if (/\.(md|puml|json)$/i.test(entry))
                    out.push(full);
            }
            return out;
        };
        ...
        const crlfDigest = getReviewInputDigest(sample, cfg.handoffPolicy);
