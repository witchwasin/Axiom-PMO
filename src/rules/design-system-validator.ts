...
interface DeclaredToken {
  name: string;
  value: string;
  section: string;
}

export function getDesignSystemDeclaredTokens(markdownText: string): DeclaredToken[] {
  const tokens: DeclaredToken[] = [];
  if (!markdownText?.trim()) return tokens;
  ...
  for (const line of lines) {
    ...
    if (tokenIndex < 0) {
      for (let i = 0; i < cells.length; i++) {
        if (cells[i] === "Token") tokenIndex = i;
        if (cells[i] === "Value") valueIndex = i;
      }
      continue;
    }
    if (valueIndex < 0) continue;
    if (/^-{2,}:?$/.test(cells[tokenIndex] ?? "") || /^:?-{2,}:?$/.test(cells[tokenIndex] ?? "")) continue;
    if (cells.length <= tokenIndex || cells.length <= valueIndex) continue;
...
export function getDesignSystemSheetTokens(htmlText: string): Array<{ name: string; value: string }> {
  const tokens: Array<{ name: string; value: string }> = [];
  if (!htmlText?.trim()) return tokens;
...
export function resolveDesignSystemVarReferences(sheetTokens: Array<{ name: string; value: string }>): Record<string, string> {
  const resolved: Record<string, string> = {};
  for (const token of sheetTokens) resolved[token.name] = token.value;

  for (let pass = 0; pass < 3; pass++) {
    let changed = false;
    for (const name of Object.keys(resolved)) {
      const value = resolved[name]!;
      const expanded = value.replace(/var\(\s*--([A-Za-z][A-Za-z0-9-]*)\s*\)/g, (_m, referenced: string) => {
...
function getNormalizedTokenValue(value: string): string {
  let normalized = (value ?? "").replace(/\s+/g, " ").trim();
  normalized = normalized.replace(/#([0-9A-Fa-f]{3,8})\b/g, (_m, hex: string) => "#" + hex.toUpperCase());
  return normalized;
}

export function testDesignSystemTokens(
