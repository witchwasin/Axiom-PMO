...
interface PsCase {
  ...
  expect: string; // JSON-encoded expected return
}
...
function runPs(fn: string, args: Record<string, unknown>): unknown {
  ...
  try {
    ...
    if (r.status !== 0) throw new Error(`ps ${fn} failed: ${r.stderr}`);
...
const psFound = runPs("Find-AxiomBlock", { Text: installed }) as Record<string, unknown>;
