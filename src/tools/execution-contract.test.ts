...
function gitInit(dir: string): void {
  ...
  if (r.status !== 0) spawnSync("git", ["-C", dir, "init", "-q"], { encoding: "utf8" });
}

function writeExecFile(dir: string, rel: string, content: string): void {
  const full = join(dir, rel);
  mkdirSync(dirname(full), { recursive: true });
  writeFileSync(full, content);
}

function newExecFixture(): string {
...
function exportContract(dir: string, grant = ""): { output: string; exitCode: number } {
  return exportExecutionContract(REPO_ROOT, dir, "D-001", null, null, grant, true);
}

function contractDigestOf(dir: string): string {
...
interface ExecEnvelope {
  ...
  summary: { pass: number; warn: number; warn_blocking: number; fail: number; exit_code: number };
  results: ExecRow[];
  execution_verification: ExecVerdict;
}

function verifyResult(dir: string, resultPath?: string): { env: ExecEnvelope; exitCode: number } {
  ...
  return { env: r.envelope as unknown as ExecEnvelope, exitCode: r.exitCode };
}

function ruleIds(env: ExecEnvelope | null, level = "FAIL"): string[] {
  if (!env) return [];
  return env.results.filter((r) => r.level === level).map((r) => r.rule_id);
}

function row(env: ExecEnvelope, ruleId: string, level = "FAIL"): ExecRow | undefined {
...
function realRunRecord(dir: string): string {
  const r = runExecutionCommand(dir, "D-001", "unit tests", "echo ok");
  if (r.exitCode !== 0) throw new Error(`realRunRecord: runner failed: ${r.output}`);
  const runsDir = join(dir, ".execution/D-001/runs");
  const recordFile = readdirSync(runsDir).find((f) => f.endsWith(".json") && !f.endsWith(".sha256"));
  if (!recordFile) throw new Error(`realRunRecord: no run record produced: ${r.output}`);
...
function setDecisionLogWithDigest(
  dir: string,
  digest: string,
  opts: { decisionId?: string; testName?: string; workItem?: string; contractDigest?: string; claimType?: string } = {},
): void {
...
function baseResultFields(dir: string): BaseResultFields {
  return {
...
function writeResultDoc(dir: string, doc: Record<string, unknown>): string {
...
function newResult(dir: string, overrides: Record<string, unknown> = {}): string {
  ...
  const doc: Record<string, unknown> = {
    ...
    base_sha: String((contract as Record<string, unknown>).base_sha),
    head_sha: head,
...
function vouchedRecordDigest(dir: string): string {
  const doc = JSON.parse(readFileSync(join(dir, ".execution/D-001/EXECUTION-RESULT.json"), "utf8")) as {
    test_evidence: Array<{ run_record_path: string }>;
  };
  return sha256(readFileSync(join(dir, doc.test_evidence[0]!.run_record_path)));
}

function appendClaim(dir: string, claim: Record<string, unknown>): void {
  const resultPath = join(dir, ".execution/D-001/EXECUTION-RESULT.json");
  const doc = JSON.parse(readFileSync(resultPath, "utf8")) as { authority_claims?: unknown[] };
  doc.authority_claims = [...(doc.authority_claims ?? []), claim];
  writeFileSync(resultPath, JSON.stringify(doc));
}

function appendLogRow(dir: string, rowText: string): void {
...
test("export: contract, digest, and scope derivation", () => {
  ...
  try {
    ...
    const allowed = (contract["allowed_paths"] as string[]) ?? [];
    const prohibited = (contract["prohibited_paths"] as string[]) ?? [];
    ...
    const ga = (contract["git_authority"] as Record<string, unknown>) ?? {};
    ...
    const reqs = (contract["requirement_refs"] as string[]) ?? [];
...
test("grant: -Grant is the only way to widen authority", () => {
  ...
  try {
    ...
    const contract = JSON.parse(readFileSync(join(dir, ".execution/D-001/EXECUTION-CONTRACT.json"), "utf8")) as Record<string, unknown>;
    const ga = (contract["git_authority"] as Record<string, unknown>) ?? {};
...
test("tampered contract: EXEC-002, verdict contract_tampered", () => {
  ...
  try {
    ...
    const contractPath = join(dir, ".execution/D-001/EXECUTION-CONTRACT.json");
    const doc = JSON.parse(readFileSync(contractPath, "utf8")) as Record<string, unknown>;
    (doc["git_authority"] as Record<string, unknown>)["push"] = true;
  ...
  try {
    ...
    assert.ok(
      exec007 !== undefined
        ...
        && exec007.documentation_url !== undefined,
      "diagnostics: every declared row field is present",
    );
...
test("vouch bound to a different test does not promote this one: EXEC-005", () => {
  ...
  try {
    ...
    newResult(dir);
    const resultPath = join(dir, ".execution/D-001/EXECUTION-RESULT.json");
    const rd = JSON.parse(readFileSync(resultPath, "utf8")) as { authority_claims: Array<Record<string, unknown>> };
...
test("legacy unbound vouch fails closed: EXEC-005, reason names no test", () => {
  ...
  try {
    ...
    newResult(dir);
    const resultPath = join(dir, ".execution/D-001/EXECUTION-RESULT.json");
    const rd = JSON.parse(readFileSync(resultPath, "utf8")) as { authority_claims?: unknown[] };
...
test("tampered run record: EXEC-005, reason names the mismatch", () => {
  ...
  try {
    ...
    const record = JSON.parse(readFileSync(recordFull, "utf8")) as { exit_code: number };
    record.exit_code = 1;
...
test("run record for wrong work item: EXEC-005", () => {
  ...
  try {
    ...
    const record = JSON.parse(readFileSync(recordFull, "utf8")) as { work_item_id: string };
...
test("real failing command: EXEC-005 (sealed exit code was 1)", () => {
  ...
  try {
    ...
    if (!recordFile) throw new Error(`no run record produced: ${run.output}`);
