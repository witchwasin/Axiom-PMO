...
function testSensitivePathMatch(path, pattern) {
...
function testExternalizationScanFinding(project, artifactPaths, secretPatterns, sensitivePathPatterns) {
    const root = resolve(project);
    for (const relative of artifactPaths) {
        const normalized = relative.replace(/\\/g, "/");
        for (const pathPattern of sensitivePathPatterns) {
            if (testSensitivePathMatch(normalized, pathPattern))
                return true;
        }
        ...
        for (const entry of secretPatterns) {
            if (entry.pattern && new RegExp(entry.pattern).test(content))
                return true;
        }
    }
    return false;
}
export function testExternalizationRegistry(acc, catalog, project, gate, orchestrationPolicy, policy, decisionIds, handoffPolicy) {
    const extPolicy = orchestrationPolicy["externalization"] ?? {};
    ...
    const humanReviewRequired = extPolicy["human_review_required"] ?? [];
    const secretPatterns = extPolicy["secret_patterns"] ?? [];
    let sensitivePathPatterns = policy["permissions"]?.["sensitive_paths"];
    ...
    const providerTypes = extPolicy["provider_types"] ?? [];
    const classifications = extPolicy["classifications"] ?? [];
    const scanResults = extPolicy["scan_results"] ?? [];
    const statuses = extPolicy["statuses"] ?? [];
    ...
    for (const entry of entries) {
        ...
        const missing = required.filter((f) => {
            const v = entry[f];
            if (Array.isArray(v) || typeof v === "object" && v !== null || typeof v === "boolean")
                return false;
        ...
        if (typeof entry.network_transfer_occurred !== "boolean")
            structureProblems.push(`${id} network_transfer_occurred`);
        if (typeof entry.human_review_required !== "boolean")
            structureProblems.push(`${id} human_review_required`);
        if (!providerTypes.includes(entry.provider_type ?? ""))
            structureProblems.push(`${id} provider_type`);
        if (!classifications.includes(entry.classification ?? ""))
            structureProblems.push(`${id} classification`);
        if (!scanResults.includes(entry.scan_result ?? ""))
            structureProblems.push(`${id} scan_result`);
        if (!statuses.includes(entry.status ?? ""))
            structureProblems.push(`${id} status`);
        const recordedAt = entry.recorded_at ?? "";
        const recordedOk = /^\d{4}-\d{2}-\d{2}T/.test(recordedAt) || testDateValue(recordedAt);
        if (!recordedOk)
            structureProblems.push(`${id} recorded_at`);
        if (String(entry.minimization_redaction ?? "").trim() === "" || testPlaceholderValue(String(entry.minimization_redaction ?? "")))
            structureProblems.push(`${id} minimization`);
        if (String(entry.purpose ?? "").trim() === "" || testPlaceholderValue(String(entry.purpose ?? "")))
            structureProblems.push(`${id} purpose`);
        ...
        let requiresHuman = humanReviewRequired.includes(entry.classification ?? "") ||
            ...
            reviewFlagProp === true;
        if (!requiresHuman && entry.classification === "Internal" && internalDefaultReview)
            requiresHuman = true;
        if (requiresHuman && reviewFlagProp !== true)
            authorityProblems.push(`${id} review flag`);
        if (requiresHuman) {
    ...
    if (structureProblems.length)
        addResult(acc, catalog, "FAIL", `Externalization registry has invalid entries: ${[...new Set(structureProblems)].join(", ")}`, { ruleId: "EXT-001", artifact: "EXTERNALIZATION.json" });
    else
        addResult(acc, catalog, "PASS", "Externalization registry structure and artifact references are valid", { ruleId: "EXT-001" });
    if (authorityProblems.length)
        addResult(acc, catalog, "FAIL", `Externalization entries lack required Human evidence: ${[...new Set(authorityProblems)].join(", ")}`, { ruleId: "EXT-002", artifact: "EXTERNALIZATION.json" });
    else
        addResult(acc, catalog, "PASS", "Required externalization transfers carry named-Human decision evidence", { ruleId: "EXT-002" });
    if (scanProblems.length)
        addResult(acc, catalog, "FAIL", `Declared clean scan does not match a deterministic re-scan of outgoing artifacts: ${[...new Set(scanProblems)].join(", ")}`, { ruleId: "EXT-003", artifact: "EXTERNALIZATION.json" });
    else
        addResult(acc, catalog, "PASS", "Declared externalization scan results match a deterministic re-scan", { ruleId: "EXT-003" });
    if (freshnessProblems.length)
        addResult(acc, catalog, "FAIL", `Externalization artifact digests are missing or stale: ${[...new Set(freshnessProblems)].join(", ")}`, { ruleId: "EXT-004", artifact: "EXTERNALIZATION.json" });
    else
        addResult(acc, catalog, "PASS", "Externalization artifact digests are current", { ruleId: "EXT-004" });
}
