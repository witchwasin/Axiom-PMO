...
import { readScopeDeclaration, readScopeDiffPolicy, resolveScopeVerdict, convertToScopeGlobRegex, } from "./scope-diff-matcher.js";
...
function resolveCombinedVerdict(a, b) {
    if (!b)
        return a;
...
function emptyResult(base, head, verdict) {
    return {
        base_sha: base,
        head_sha: head,
        approved_include: [],
...
export function invokeScopeDiffCheck(acc, catalog, projectPath, gitRepoRoot, frameworkRoot, baseRef, headRef) {
    ...
    const verdict = outOfScope.length > 0 || excluded.length > 0 ? "fail" : "pass";
    if (verdict === "pass") {
